...
class WBGameDataScreen:
	...
	def placeGameOptions(self):
		...
		#SpyFanatic: add also victories   X,Y,width,height
		iWidth = screen.getXResolution()/2 - 40
		screen.addTableControlGFC("WBVictoryOptions", 2, screen.getXResolution()/2 + 20, self.iScript_Y -25, iWidth, 100, False, False, 24, 24, TableStyles.TABLE_STYLE_STANDARD)
		screen.setTableColumnHeader("WBVictoryOptions", 0, "", iWidth)

		for item in xrange(gc.getNumVictoryInfos()):
			screen.appendTableRow("WBVictoryOptions")
			Victory = gc.getVictoryInfo(item)
			iColumn = 0
			iRows = item
			sColor = CyTranslator().getText("[COLOR_WARNING_TEXT]", ())
			if CyGame().isVictoryValid(item):
				sColor = CyTranslator().getText("[COLOR_POSITIVE_TEXT]", ())
			sText = "<font=3>" + sColor + Victory.getDescription() + "</font></color>"
			screen.setTableText("WBVictoryOptions", iColumn, iRows, sText, "", WidgetTypes.WIDGET_PYTHON, 1028, item, CvUtil.FONT_LEFT_JUSTIFY)
	...
	def handleInput(self, inputClass):
		...
		elif inputClass.getFunctionName() == "WBVictoryOptions":
			iGameOption = inputClass.getData2()
			CyGame().setVictoryValid(iGameOption, not CyGame().isVictoryValid(iGameOption))
			#self.checkOptions(iGameOption)
			self.placeGameOptions()
	...
	def checkOptions(self, iGameOption):
		if iGameOption == GameOptionTypes.GAMEOPTION_LEAD_ANY_CIV:
			...
			iSelectedLeader = -1
			self.interfaceScreen()
		#SpyFanatic: not existing in MoM
