...
#added Sephi									
	...
	#SpyFanatics iFreezeBuy disable possibility to buy and lock the possibility to switch between screen
	def interfaceScreen(self, iScreen, iFreezeBuy):
...
#		screen.setDimensions(screen.centerX(0), screen.centerY(0), 1024, 768)
		...
		if (self.REV_SCREEN_DICT.has_key(self.iScreen)):
			self.DRAW_DICT[self.REV_SCREEN_DICT[self.iScreen]] (iFreezeBuy)
		else:
			return
	...
	def drawGuilds (self,iFreezeBuy):
		# Place the tech blocks
		self.placeTechs(iFreezeBuy)
	
		# Draw the arrows
		self.drawArrows()


	def drawTraits (self,iFreezeBuy):	
		screen = CyGInterfaceScreen( "Guilds", CvScreenEnums.GUILD_SCREEN )
		...
		for i in range(gc.getNumTraitInfos()):
			if pPlayer.canGetTrait(i) and iFreezeBuy == 0:
...
# todo: use cost formula
	...
	def placeTechs (self,iFreezeBuy):
		...
		for i in range(gc.getNumTechInfos()):
			...
			#Xtended taking alignment into consideration for Guild techs
			...
			if i == iWayofWise:
				...
				if pPlayer.isHasTech(iWayofWise):
					bCanResearchEver = false
			elif i == iRighteousness:
				if pPlayer.isHasTech(iCommune) or pPlayer.isHasTech(iMalevolent):
					bCanResearchEver = false
			elif i == iCommune:
			...
			if (bCanResearchEver and iFreezeBuy == 0):
			...
			elif iFreezeBuy == 1 or not (bCanResearchEver):
				screen.attachPanelAt( "TechList", szTechRecord, u"", u"", True, False, PanelStyles.PANEL_STYLE_TECH, iX - 6, iY - 6, self.getXStart() + 6, 12 + ( BOX_INCREMENT_HEIGHT * PIXEL_INCREMENT ), WidgetTypes.WIDGET_GENERAL, i, iCapitalID )
			...
			elif not (bCanResearchEver):
				screen.setPanelColor(szTechRecord, 0, 0, 0)
				self.aiCurrentState.append(CIV_NO_TECH)
			elif (iFreezeBuy == 0 and gc.getPlayer(self.iCivSelected).canGetCulturalAchievement(i)):
				if (bCanResearchEver):
