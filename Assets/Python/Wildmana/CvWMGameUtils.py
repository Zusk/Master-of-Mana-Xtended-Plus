...
#			iValue = self.AI_CivicValue_Economy(ePlayer, eCivic)
	...
	def AI_CivicValue_Government(self, ePlayer, eCivic):
		...
		bArcane = pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_ARCANE'))
...
#				iValue += 2500
	...
	def AI_CivicValue_CulturalValues(self, ePlayer, eCivic):
		...
		bNoUnhappy = false
		bFallow = pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_FALLOW'))
		bFaithful = pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_FAITHFUL'))
		...
		if eCivic == gc.getInfoTypeForString('CIVIC_HARVESTING'):
			iValue = 1000
			if gc.getCivilizationInfo(eCiv).getImpInfrastructureHalfCost() == gc.getInfoTypeForString('IMPROVEMENT_CAMP'):
				iValue += 1000
			if bAristo:
				iValue += 1000
...
#				iValue = 10000
	...
	def AI_CivicValue_Labor(self, ePlayer, eCivic):
		...
		if eAIEcon == AIEconomyTypes.AIECONOMY_FOL:
			bFolEcon = true
	...
	def AI_CivicValue_Membership(self, ePlayer, eCivic):
		...
		if eCivic == gc.getInfoTypeForString('CIVIC_OVERCOUNCIL'):
			if pPlayer.getCivics(gc.getInfoTypeForString('CIVICOPTION_CULTURAL_VALUES')) == gc.getInfoTypeForString('CIVIC_BENEVOLENCE'):
				iValue = +1000
			elif pPlayer.getCivics(gc.getInfoTypeForString('CIVICOPTION_CULTURAL_VALUES')) == gc.getInfoTypeForString('CIVIC_SLAVERY'):
				iValue = -1000
			else:
				iValue = 500
			...
			if eAlignment == gc.getInfoTypeForString('ALIGNMENT_EVIL'):
				if pPlayer.getCivics(gc.getInfoTypeForString('CIVICOPTION_CULTURAL_VALUES')) == gc.getInfoTypeForString('CIVIC_SLAVERY'):
					iValue = +500
				if pPlayer.getCivics(gc.getInfoTypeForString('CIVICOPTION_CULTURAL_VALUES')) == gc.getInfoTypeForString('CIVIC_SACRIFICE_THE_WEAK'):
					iValue = +500
			else:
				iValue = 500
