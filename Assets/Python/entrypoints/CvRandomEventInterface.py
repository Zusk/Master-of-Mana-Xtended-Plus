...
def doBanditNietz3(argsList):
	...
	newUnit = pPlayer.initUnit(gc.getInfoTypeForString('UNIT_MERCENARY_MOUNTED'), pCity.getX(), pCity.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
	...
	newUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_RAIDER1'), True)
	newUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_RAIDER2'), True)
	newUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_RAIDER3'), True)
...
def doTraitAggressive(argsList):
	...
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_AGGRESSIVE_ADAPTIVE'),True)
	...
	for iTrait in range(gc.getNumTraitInfos()):
		if pPlayer.hasTrait(iTrait):
			if (gc.getTraitInfo(iTrait).isSelectable()):
				if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() != iTrait:
					pPlayer.setHasTrait(iTrait,False)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_ARCANE_ADAPTIVE'),True)
	...
	for iTrait in range(gc.getNumTraitInfos()):
		if pPlayer.hasTrait(iTrait):
			if (gc.getTraitInfo(iTrait).isSelectable()):
				if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() != iTrait:
					pPlayer.setHasTrait(iTrait,False)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_CHARISMATIC_ADAPTIVE'),True)
	...
	for iTrait in range(gc.getNumTraitInfos()):
		if pPlayer.hasTrait(iTrait):
			if (gc.getTraitInfo(iTrait).isSelectable()):
				if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() != iTrait:
					pPlayer.setHasTrait(iTrait,False)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_CREATIVE_ADAPTIVE'),True)
	
def canApplyTraitDefender(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() == gc.getInfoTypeForString('TRAIT_DEFENDER'):
		return False
	return True

def doTraitDefender(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	for iTrait in range(gc.getNumTraitInfos()):
		if pPlayer.hasTrait(iTrait):
			if (gc.getTraitInfo(iTrait).isSelectable()):
				if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() != iTrait:
					pPlayer.setHasTrait(iTrait,False)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_DEFENDER_ADAPTIVE'),True)
	...
	for iTrait in range(gc.getNumTraitInfos()):
		if pPlayer.hasTrait(iTrait):
			if (gc.getTraitInfo(iTrait).isSelectable()):
				if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() != iTrait:
					pPlayer.setHasTrait(iTrait,False)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_EXPANSIVE_ADAPTIVE'),True)
	
def canApplyTraitFaithful(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() == gc.getInfoTypeForString('TRAIT_FAITHFUL'):
		return False
	return True

def doTraitFaithful(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	for iTrait in range(gc.getNumTraitInfos()):
		if pPlayer.hasTrait(iTrait):
			if (gc.getTraitInfo(iTrait).isSelectable()):
				if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() != iTrait:
					pPlayer.setHasTrait(iTrait,False)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_FAITHFUL_ADAPTIVE'),True)
	...
	for iTrait in range(gc.getNumTraitInfos()):
		if pPlayer.hasTrait(iTrait):
			if (gc.getTraitInfo(iTrait).isSelectable()):
				if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() != iTrait:
					pPlayer.setHasTrait(iTrait,False)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_FINANCIAL_ADAPTIVE'),True)
	...
	for iTrait in range(gc.getNumTraitInfos()):
		if pPlayer.hasTrait(iTrait):
			if (gc.getTraitInfo(iTrait).isSelectable()):
				if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() != iTrait:
					pPlayer.setHasTrait(iTrait, False)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_INDUSTRIOUS_ADAPTIVE'),True)
...
#Sephi
	...
	Traits = [ 'TRAIT_AGGRESSIVE_ADAPTIVE','TRAIT_ARCANE_ADAPTIVE','TRAIT_CHARISMATIC_ADAPTIVE','TRAIT_CREATIVE_ADAPTIVE','TRAIT_DEFENDER_ADAPTIVE','TRAIT_EXPANSIVE_ADAPTIVE','TRAIT_FAITHFUL_ADAPTIVE','TRAIT_FINANCIAL_ADAPTIVE','TRAIT_INDUSTRIOUS_ADAPTIVE','TRAIT_ORGANIZED_ADAPTIVE','TRAIT_PHILOSOPHICAL_ADAPTIVE','TRAIT_RAIDERS_ADAPTIVE','TRAIT_SPIRITUAL_ADAPTIVE' ]
	...
	for iTrait in range(gc.getNumTraitInfos()):
		if pPlayer.hasTrait(iTrait):
			if (gc.getTraitInfo(iTrait).isSelectable()):
				if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() != iTrait:
					pPlayer.setHasTrait(iTrait, False)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_ORGANIZED_ADAPTIVE'),True)
	...
	for iTrait in range(gc.getNumTraitInfos()):
		if pPlayer.hasTrait(iTrait):
			if (gc.getTraitInfo(iTrait).isSelectable()):
				if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() != iTrait:
					pPlayer.setHasTrait(iTrait, False)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_PHILOSOPHICAL_ADAPTIVE'),True)
	...
	for iTrait in range(gc.getNumTraitInfos()):
		if pPlayer.hasTrait(iTrait):
			if (gc.getTraitInfo(iTrait).isSelectable()):
				if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() != iTrait:
					pPlayer.setHasTrait(iTrait, False)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_RAIDERS_ADAPTIVE'),True)
	...
	for iTrait in range(gc.getNumTraitInfos()):
		if pPlayer.hasTrait(iTrait):
			if (gc.getTraitInfo(iTrait).isSelectable()):
				if gc.getLeaderHeadInfo(pPlayer.getLeaderType()).getPermanentTrait() != iTrait:
					pPlayer.setHasTrait(iTrait, False)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_SPIRITUAL_ADAPTIVE'),True)
...
def canApplyVolcano1(argsList):
	...
	if (iNumImprovements > 1):	#Don't turn the only peak into a volcano.
		return true
	else:
		return false
...
def applyEarthDay2(argsList):
	...
	iCivic = CvUtil.findInfoTypeNum(gc.getCivicInfo,gc.getNumCivicInfos(),'CIVIC_GUARDIAN_OF_NATURE')
	iCivicOption = CvUtil.findInfoTypeNum(gc.getCivicOptionInfo,gc.getNumCivicOptionInfos(),'CIVICOPTION_LABOR')
...
def spawnLocalLegend(argsList):
	...
	if iCiv==gc.getInfoTypeForString('CIVILIZATION_ELOHIM') or iCiv==gc.getInfoTypeForString('CIVILIZATION_KHAZAD') or iCiv==gc.getInfoTypeForString('CIVILIZATION_INFERNAL') or iCiv==gc.getInfoTypeForString('CIVILIZATION_DOVIELLO') or iCiv==gc.getInfoTypeForString('CIVILIZATION_CALABIM'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_CIV_CITY_ATTACKER')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_AMURITES') or iCiv==gc.getInfoTypeForString('CIVILIZATION_LJOSALFAR') or iCiv==gc.getInfoTypeForString('CIVILIZATION_SVARTALFAR') or iCiv==gc.getInfoTypeForString('CIVILIZATION_AUSTRIN'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_CIV_MOUNTED')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_KURIOTATES'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_CIV_ARCHER')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_BANNOR') or iCiv==gc.getInfoTypeForString('CIVILIZATION_CLAN_OF_EMBERS') or iCiv==gc.getInfoTypeForString('CIVILIZATION_ILLIANS'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_CIV_RECON')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_MALAKIM') or iCiv==gc.getInfoTypeForString('CIVILIZATION_SIDAR'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_CIV_UU1')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_MAZATL'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_PRIEST_OF_OMORR')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_BALSERAPHS') or iCiv==gc.getInfoTypeForString('CIVILIZATION_LANUN'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_CIV_UU2')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_GRIGORI') or iCiv==gc.getInfoTypeForString('CIVILIZATION_HIPPUS'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_ADVENTURER')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_LUCHUIRP'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_BATTLECAPTAIN')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_MERCURIANS'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_HERALD')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_SHEAIM'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_SUCCUBUS')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_ARISTRAKH'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_CIV_DEFAULT')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_FAERIES'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_SUN_FAERIE')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_SCIONS'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_RANGER')
	elif iCiv==gc.getInfoTypeForString('CIVILIZATION_DURAL'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_PROFESSOR')
	else:
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_CIV_DEFAULT')
...
def canDoCondatis1(argsList):
	kTriggeredData = argsList[0]
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	iUnit = gc.getInfoTypeForString('UNIT_CONDANTIS')
	if CyGame().getUnitCreatedCount(iUnit) > 0:
		return False
	if pPlayer.getAlignment() == gc.getInfoTypeForString('ALIGNMENT_EVIL'):
		return False
	...
	for iPlayer2 in range(gc.getMAX_PLAYERS()):
		pPlayer2 = gc.getPlayer(iPlayer2)
		iTemple = CvUtil.findInfoTypeNum(gc.getBuildingClassInfo, gc.getNumBuildingClassInfos(), 'BUILDINGCLASS_TEMPLE_OF_REMEMBRANCE')
		if pPlayer2.getBuildingClassCount(iTemple) > 0:
			return False
	if not pPlayer.isHuman():
		for iPlayer2 in range(gc.getMAX_PLAYERS()):
			pPlayer2 = gc.getPlayer(iPlayer2)
			if pPlayer2.isHuman():
				if pPlayer2.getAlignment() != gc.getInfoTypeForString('ALIGNMENT_EVIL'):
					if pPlayer2.getStateReligion() != (gc.getInfoTypeForString('RELIGION_OCTOPUS_OVERLORDS')):
						if not pPlayer2.hasTrait(gc.getInfoTypeForString('TRAIT_AGNOSTIC')):
							return False
	return True

def canDoCondatis2(argsList):
	kTriggeredData = argsList[0]
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	iUnit = gc.getInfoTypeForString('UNIT_CONDANTIS')
	...
	if pPlayer.getAlignment() == gc.getInfoTypeForString('ALIGNMENT_EVIL'):
		return False
	if (pPlayer.getStateReligion() == (gc.getInfoTypeForString('RELIGION_OCTOPUS_OVERLORDS')) or pPlayer.getStateReligion() == (gc.getInfoTypeForString('RELIGION_THE_ASHEN_VEIL'))):
		return False
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_AGNOSTIC')):
		return False
	return True

def doCondantis11(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	...
	for iPlayer2 in range(gc.getMAX_CIV_PLAYERS()):
		pPlayer2 = gc.getPlayer(iPlayer2)
		if (pPlayer2.getStateReligion() == (gc.getInfoTypeForString('RELIGION_OCTOPUS_OVERLORDS'))):
			pPlayer2.AI_changeAttitudeExtra(iPlayer,2)

def helpCondantis11(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	szHelp = localText.getText("TXT_KEY_EVENT_CONDANTIS_1_1_HELP", ())
	return szHelp
	
def doCondantis12(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
...
def helpCondantis12(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	szHelp = localText.getText("TXT_KEY_EVENT_CONDANTIS_1_2_HELP", ())
	return szHelp
	
def doCondantis21(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
...
def helpCondantis21(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	szHelp = localText.getText("TXT_KEY_EVENT_CONDANTIS_2_1_HELP", ())
	return szHelp
	
def doCondantis22(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	...
	for iPlayer2 in range(gc.getMAX_CIV_PLAYERS()):
		pPlayer2 = gc.getPlayer(iPlayer2)
		if (pPlayer2.isAlive() and pPlayer2.getStateReligion() == (gc.getInfoTypeForString('RELIGION_OCTOPUS_OVERLORDS'))):
			pPlayer2.AI_changeAttitudeExtra(iPlayer,-6)
			gc.getTeam(pPlayer2.getTeam()).declareWar(pPlayer.getTeam(), false, WarPlanTypes.WARPLAN_LIMITED)
			if not pPlayer2.isHuman():
				newUnit = pPlayer2.initUnit(gc.getInfoTypeForString('UNIT_DROWN'), pPlayer2.getCapitalCity().getX(), pPlayer2.getCapitalCity().getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
				newUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_HERO'), True)
				newUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_CANNIBALIZE'), True)
				newUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_STRONG'), True)
				newUnit.setName("Guardian of the Deep")

def helpCondantis22(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	szHelp = localText.getText("TXT_KEY_EVENT_CONDANTIS_2_2_HELP", ())
	return szHelp

def candoRemembrance(argsList):
	kTriggeredData = argsList[0]
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	if pPlayer.getUnitClassCount(CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_CHILD_OF_CONDANTIS')) < 4:
		if (pPlayer.getStateReligion() != (gc.getInfoTypeForString('RELIGION_OCTOPUS_OVERLORDS')) and pPlayer.getStateReligion() != (gc.getInfoTypeForString('RELIGION_THE_ASHEN_VEIL')) and pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_AGNOSTIC')) == False):
			return True
	return False
