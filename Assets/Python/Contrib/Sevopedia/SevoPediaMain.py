			...
			for iPass in range(100):
				...
				if bTrait:
					...
					if iPass == 4:
						...
						infoList.append( (szDescription, 0, szDescription) )
					if iPass == 5:
						szDescription = localText.getText("TXT_KEY_PEDIA_TRAITS_FAITHFUL",		())			
						infoList.append( (szDescription, 0, szDescription) )						
					if iPass == 6:
						...
						if getInfo(iItemID):
							...
							if(gc.getTraitInfo(iItemID).getPrereqTrait() != -1):
								...
								if gc.getTraitInfo(iItemID).getPrereqTrait() == gc.getInfoTypeForString('TRAIT_AGGRESSIVE'):
									...
									bArcaneTrait = true
								elif gc.getTraitInfo(iItemID).getPrereqTrait() == gc.getInfoTypeForString('TRAIT_FAITHFUL'):
									bFaithfulTrait = true
								else:
									bBasicTrait = true
							...
							if (bImperialTrait and iPass == 4):
								bValid = true
							if (bFaithfulTrait and iPass == 5):
								bValid = true

							if (bCivTrait and iPass == 6):
								bValid = true
							...
							if bValid:
								if ((None in (iCategory, getCategory) or getCategory(iItemID) == iCategory) and not getInfo(iItemID).isGraphicalOnly()):
									...
									if szDescription[0:4] == "The ":
										...
										infoList.append( (szDescription, iItemID, szDescription) )
			if bTrait:
				#Link the pre-req traits to the actual trait documented under the basic trait category
				for index, value in enumerate(infoList):
					if value[0] == getInfo(gc.getInfoTypeForString('TRAIT_AGGRESSIVE')).getDescription():
						for localTextIndex, localTextValue in enumerate(infoList):
							if localTextValue[0] == localText.getText("TXT_KEY_PEDIA_TRAITS_AGGRESSIVE",		()):
								infoList[localTextIndex] = (localTextValue[0], value[1], localTextValue[2])
					if value[0] == getInfo(gc.getInfoTypeForString('TRAIT_ARCANE')).getDescription():
						for localTextIndex, localTextValue in enumerate(infoList):
							if localTextValue[0] == localText.getText("TXT_KEY_PEDIA_TRAITS_ARCANE",		()):
								infoList[localTextIndex] = (localTextValue[0], value[1], localTextValue[2])
					if value[0] == getInfo(gc.getInfoTypeForString('TRAIT_CREATIVE')).getDescription():
						for localTextIndex, localTextValue in enumerate(infoList):
							if localTextValue[0] == localText.getText("TXT_KEY_PEDIA_TRAITS_CREATIVE",		()):
								infoList[localTextIndex] = (localTextValue[0], value[1], localTextValue[2])
					if value[0] == getInfo(gc.getInfoTypeForString('TRAIT_IMPERIALIST')).getDescription():
						for localTextIndex, localTextValue in enumerate(infoList):
							if localTextValue[0] == localText.getText("TXT_KEY_PEDIA_TRAITS_IMPERIAL",		()):
								infoList[localTextIndex] = (localTextValue[0], value[1], localTextValue[2])
					if value[0] == getInfo(gc.getInfoTypeForString('TRAIT_FAITHFUL')).getDescription():
						for localTextIndex, localTextValue in enumerate(infoList):
							if localTextValue[0] == localText.getText("TXT_KEY_PEDIA_TRAITS_FAITHFUL",		()):
								infoList[localTextIndex] = (localTextValue[0], value[1], localTextValue[2])		
								
				#Link the non-pre-req traits like civilization and leader traits to the first trait listed under the category
				for index, value in enumerate(infoList):
					if value[0] == localText.getText("TXT_KEY_PEDIA_TRAITS_BASIC",		()) or \
						value[0] == localText.getText("TXT_KEY_PEDIA_TRAITS_CIVTRAIT",		()) or \
						value[0] == localText.getText("TXT_KEY_PEDIA_TRAITS_LEADER",		()):
						infoList[index] = (infoList[index][0], infoList[index + 1][1], infoList[index][2])
