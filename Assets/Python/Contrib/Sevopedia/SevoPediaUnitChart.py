...
#BUGFfH: End Add
	...
	def placeUnitTable(self):
		...
		#screen.addTableControlGFC(szTable, 5, self.X_UNITS + self.MARGIN, self.Y_UNITS + self.MARGIN + 5, self.W_UNITS - (self.MARGIN * 2), self.H_UNITS - (self.MARGIN * 2) - 5, True, False, 32,32, TableStyles.TABLE_STYLE_EMPTY)
		screen.addTableControlGFC(szTable, 7, self.X_UNITS + self.MARGIN, self.Y_UNITS + self.MARGIN + 5, self.W_UNITS - (self.MARGIN * 2), self.H_UNITS - (self.MARGIN * 2) - 5, True, False, 32,32, TableStyles.TABLE_STYLE_EMPTY)
		...
		#iColWidth = int(iTableWidth * (3 / 19.0))
		iColWidth = int(iTableWidth * (2 / 19.0))
		...
		screen.setTableColumnHeader(szTable, 4, u"%c" % CyGame().getSymbolID(FontSymbols.SILVER_STAR_CHAR), iColWidth)
		screen.setTableColumnHeader(szTable, 5, u"%c" % CyGame().getSymbolID(FontSymbols.STAR_CHAR), iColWidth)
		screen.setTableColumnHeader(szTable, 6, u"%c" % CyGame().getSymbolID(FontSymbols.MAP_CHAR), iColWidth)
		nUnits = 0
		...
		unitsList=[(0,0,0,0,0,0,0,0)]*nUnits
		for j in range(gc.getNumUnitInfos()):
			if (self.iGroup == gc.getUnitInfo(j).getUnitCombatType() or self.iGroup == gc.getNumUnitCombatInfos()):
				...
				unitsList[i] = (gc.getUnitInfo(j).getCombat(), gc.getUnitInfo(j).getMoves(), szCost, gc.getUnitInfo(j).getDescription(), j, gc.getUnitInfo(j).getTier(), gc.getUnitInfo(j).getStateReligion(), gc.getUnitInfo(j).getPrereqCiv())
				i += 1
		for i in range(nUnits):			
			...
			screen.setTableInt(szTable, 4, iRow, u"<font=3>" + unicode(unitsList[i][5]) + u"</font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
			screen.setTableInt(szTable, 5, iRow, u"<font=3>" + unicode(unitsList[i][6]) + u"</font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
			screen.setTableInt(szTable, 6, iRow, u"<font=3>" + unicode(unitsList[i][7]) + u"</font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
