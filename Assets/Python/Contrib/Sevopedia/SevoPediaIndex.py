...
##--------	BUGFfH: Modified by Denev 2009/10/09
		#I suspect the coloration of the linked traits means that they don't fit into a standard A-Z category well which causes a problem with the index display in the Civilopedia.  
		#Removing them fixes the issue which is fine because they aren't proper traits anyway, despite some of them having corresponding base traits.
		for szIndexName, iItemID, szItemName in traitList:
			if szIndexName == localText.getText("TXT_KEY_PEDIA_TRAITS_BASIC",		()):
				traitList.remove((szIndexName, iItemID, szItemName))
			if szIndexName == localText.getText("TXT_KEY_PEDIA_TRAITS_AGGRESSIVE",	()):
				traitList.remove((szIndexName, iItemID, szItemName))
			if szIndexName == localText.getText("TXT_KEY_PEDIA_TRAITS_ARCANE",		()):
				traitList.remove((szIndexName, iItemID, szItemName))
			if szIndexName == localText.getText("TXT_KEY_PEDIA_TRAITS_CREATIVE",	()):
				traitList.remove((szIndexName, iItemID, szItemName))
			if szIndexName == localText.getText("TXT_KEY_PEDIA_TRAITS_IMPERIAL",	()):
				traitList.remove((szIndexName, iItemID, szItemName))
			if szIndexName == localText.getText("TXT_KEY_PEDIA_TRAITS_FAITHFUL",	()):
				traitList.remove((szIndexName, iItemID, szItemName))
			if szIndexName == localText.getText("TXT_KEY_PEDIA_TRAITS_CIVTRAIT",	()):
				traitList.remove((szIndexName, iItemID, szItemName))
