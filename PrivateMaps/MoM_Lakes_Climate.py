...
def useClimateBasedStartPos():
	"This map should use the climate based starting positions and terrain generation"
	return 1
...
class LakesFractalWorld(CvMapGeneratorUtil.FractalWorld):
	def generatePlotTypes(self, water_percent=20, shift_plot_types=True, 
	                      grain_amount=3):
		...
		water_percent = min(water_percent, 30)
		water_percent = max(water_percent, 15)
...
def generatePlotTypes():
	...
	fractal_world.initFractal(continent_grain = 3, rift_grain = -1, has_center_rift = False, invert_heights = True, polar = False)
	plot_types = fractal_world.generatePlotTypes(water_percent = 20)
	return plot_types
