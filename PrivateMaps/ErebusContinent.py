...
## GLOBAL TUNING VARIABLES: Change these to customize the map results
		...
		#players starting units to all be on the same tile like it does for the
		#AI players.
		...
		self.itNoFeature = -1 #GetInfoType("NO_FEATURE")
		self.itFeatureIce = GetInfoType("FEATURE_ICE")
		self.itFeatureKelp = GetInfoType("FEATURE_KELP")
		self.itFeatureReefs = -1 #GetInfoType("FEATURE_REEFS")
		...
		self.itFeatureCrystalPlains = -1 #GetInfoType("FEATURE_CRYSTAL_PLAINS")

		if self.itFeatureHauntedLands == -1:
			self.itFeatureHauntedLands = GetInfoType("FEATURE_FOREST_BURNT")

		self.itNoImprovement = -1 #GetInfoType("NO_IMPROVEMENT")
		self.itImprovementCityRuinsAncient = -1 #GetInfoType("IMPROVEMENT_CITY_RUINS_ANCIENT")
...
def getCustomMapOptionDescAt(argsList):
	...
	elif optionID == 13: #Bonuses
		if selectionID == 0:
			return "Yes"
		elif selectionID == 1:
			return "No - Only mana survived the age of ice."
	return u""
			...
			for x in range(5,mc.width-5):
				...
				if plot.getArea() == self.areaID and not plot.isWater():
					...
					if plot.getBonusType(TeamTypes.NO_TEAM) != BonusTypes.NO_BONUS:
						continue
					if plot.isPeak(): # Some mods have peaks with a positive value here
						continue
					desertValue = 0
					plainsValue = 0
					grassValue = 0
...
class BonusPlacer :
	...
	def AddBonusType(self,eBonus):
		...
		if gameMap.getCustomMapOption(13) == 1 and bonusInfo.getType() != "BONUS_MANA":
			bonus.desiredBonusCount = 0
...
def getPlaneCreationMethod():
	# 0 = neighbour planes, 1 = Neihghbour planes mixed
	# 2 = continent planes, 3 = only one plane
	return 3
