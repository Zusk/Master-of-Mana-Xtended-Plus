...
import random
import sys
#sys.path.append("Mods\\Fall from Heaven 2\\PublicMaps")
#sys.path.append("E:\\Pelit\\CivIV\\Beyond the Sword\\Mods\\Fall from Heaven 2\\PublicMaps")
#import OALTerrain as TerrainGenerator
...
#FlavourMod: Added by Jean Elcard 02/04/2009
		if hasattr(WorldSizeTypes, "WORLDSIZE_GIANT"):
			grid_sizes[WorldSizeTypes.WORLDSIZE_GIANT] = (30,20)
#FlavourMod: End Add
	...
	#reset our globals whenever the map is regenerated
	def beforeInit():
		global rng
		map = CyMap()
...
#######################
		...
		featureHauntedLands = gc.getInfoTypeForString("FEATURE_HAUNTED_LANDS")
		terrainCoast = gc.getInfoTypeForString("TERRAIN_COAST")		
########################
### Orbis-friendly end##
########################
		...
		for x in range(mc.x_size):
			for y in range(mc.y_size):
				...
				if not plot.isPeak() and not plot.isWater():
					if rng() < 0.005:
						plot.setFeatureType(featureHauntedLands, 0)
