...
//1
	...
	for (iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
	{
		...
		FAssert(iForcedSpecialistCount <= 1000);
...
*/
	...
	else
	{
		...
		if ((iBaseProductionModifier + iExtraProductionModifier) != 0)
...
//			if(getCommerceRateTimes100(COMMERCE_RESEARCH)>50)
		...
		if(iI==CITYSPECIALIZATION_METAL)
		{
			iValue+= getYieldRate(YIELD_METAL)*100;
		...
		{
			iValue+= getYieldRate(YIELD_LUMBER)*100;
		...
		{
			iValue+= getYieldRate(YIELD_LEATHER)*100;
