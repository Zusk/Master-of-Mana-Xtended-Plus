...
bool CvGame::civIsOkForThisPlayer(int iPlayer, int iCiv)
{
	return (GC.getCivilizationInfo((CivilizationTypes)iCiv).isAIPlayable() && (GC.getInitCore().getSlotStatus((PlayerTypes)iPlayer) == SS_COMPUTER)) ||
		((GC.getCivilizationInfo((CivilizationTypes)iCiv).isPlayable() && (GC.getInitCore().getSlotStatus((PlayerTypes)iPlayer) == SS_TAKEN)));
}
...
void CvGame::assignStartingPlots()
{
	...
	if (GC.getMapINLINE().useClimateBasedStartPos())
	{
		...
		for (iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; iPlayer++)
		{
			...
			CvPlot* setStartPlot = GET_PLAYER((PlayerTypes)iPlayer).getStartingPlot();

			if (setStartPlot != NULL)
			{
				if (std::find(startingPosAlreadyTaken.begin(), startingPosAlreadyTaken.end(), setStartPlot) != startingPosAlreadyTaken.end()) {
					// This is already taken, find a new one anyway....
				}
				else {
					// Not taken, claim it for the good of the land!
					startingPosAlreadyTaken.push_back(setStartPlot);
					continue;
				}

			}

			CvPlot* foundStartPlot = GET_PLAYER((PlayerTypes)iPlayer).findStartingPlot(startingPosAlreadyTaken, false);

			if (foundStartPlot == NULL)
			{
				// This should not be...
				FAssertMsg(false, "A player ended up with no starting position, call the coder and spank buts!");
				continue;
			}

			startingPosAlreadyTaken.push_back(foundStartPlot);
...
{
	if (GC.getMapINLINE().useClimateBasedStartPos())
		return; // Don't mess up our well selected starting positions
...
// different Logic for Holy Cities
	...
	// Neither the old or the new method of creating holy cities are used now, the holy techs trigger this - SayWhatEh 2015-03-18 
	return;
					...
					if((abs(iX) > iRangeMin) && (abs(iY) > iRangeMin)) {
						...
						if(pLoopStart != NULL && !pLoopStart->isWater() && !pLoopStart->isPeak() 
							&& pLoopStart->getArea() == pLoopPlot->getArea()) {
								if((GC.getMapINLINE().useClimateBasedStartPos() || pLoopStart->getPlane() == iNativePlane) && !pLoopStart->isVisibleToCivTeam())
...
int CvGame::getSorenRandNum(int iNum, const char* pszLog)
{
	...
	TCHAR szOut[1024];
	sprintf(szOut,"Turn: %d (%d,%d): %s\n"
		,GC.getGameINLINE().getElapsedGameTurns()
		,iNum
		,iValue
		,pszLog
	);
	gDLL->logMsg("OOS.log",szOut, false, false);
