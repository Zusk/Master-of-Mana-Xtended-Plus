...
/*************************************************************************************************/
	...
	{
		CvPlot* pLoopPlot;
		int iRange = GC.getDefineINT("ADVANCED_START_SIGHT_RANGE");		
		...
		for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
		{
			...
			{
				pLoopPlot = kPlayer.getStartingPlot();
				if (kPlayer.isSprawling()) //Want to provide full information on a starting city in advanced start by adding an additional 2 (or 3) plot visibility.
					iExtraRevealRange = 3;
				else
					iExtraRevealRange = 2;
				...
				for (iDX = -(iRange + iExtraRevealRange); iDX <= iRange + iExtraRevealRange; iDX++)
				{
					for (iDY = -(iRange + iExtraRevealRange); iDY <= iRange + iExtraRevealRange; iDY++)
					{
						pLoopPlot = plotXY(iX, iY, iDX, iDY);

						if (pLoopPlot != NULL)
						{
							if (plotDistance(iX, iY, iX + iDX, iY + iDY) <= iRange) {
								if (iDX == 0 && iDY == 0)	//Starting plot to add the red circle
								{
