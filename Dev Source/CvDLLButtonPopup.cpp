...
/*************************************************************************************************/
	...
	case BUTTONPOPUP_CHOOSE_GUILD:
		//SpyFanatic: allow look at the big picture while pickup guild
		if (pPopupReturn->getButtonClicked() == GC.getNumTechInfos())
		{
			CyArgsList argsList;
			argsList.add(0); //guild screen
			argsList.add(1); //freeze buy
			bool bResult = gDLL->getPythonIFace()->callFunction("CvScreensInterface", "showGuildScreen", argsList.makeFunctionArgs());
			GET_PLAYER(GC.getGameINLINE().getActivePlayer()).allowedGuild();
		}
		else
		{
			//broadcast that we picked == bought the tech
