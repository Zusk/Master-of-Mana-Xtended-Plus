...
void CvSelectionGroup::doTurn()
{
	...
	if (getNumUnits() > 0)
	{
		...
		while (pUnitNode != NULL)
		{
			...
			//SpyFanatic: if unit is hurt, but cannot heal do not consider it
			if (pLoopUnit->isHurt() && pLoopUnit->healRate(pLoopUnit->plot()) > 0/*&& pLoopUnit->canHeal(plot())*/)
			{
				bHurt = true;
			}
		}
...
bool CvSelectionGroup::canDoCommand(CommandTypes eCommand, int iData1, int iData2, bool bTestVisible, bool bUseCache, CvPlot* pPlot)
{
	PROFILE_FUNC();
	...
	while (pUnitNode != NULL)
	{
		...
		if (pLoopUnit->canDoCommand(eCommand, iData1, iData2, bTestVisible, false, pPlot))
		{
			return true;
		}
//SpyFanatic: This cause systematic OOS
/*
		else {
			pLoopUnit->setMissionSpell(NO_SPELL);	//Mission spells weren't being properly cleared.
		}
*/
	}
...
/*************************************************************************************************/
		case INTERFACEMODE_SPELL_OFFENSIVE:
			if (pLoopUnit != NULL)
			{
				if(pLoopUnit->getMissionSpell() != NO_SPELL && GC.getSpellInfo((SpellTypes) pLoopUnit->getMissionSpell()).isTargetedHostile() && pLoopUnit->canCastAt((SpellTypes) pLoopUnit->getMissionSpell(),pPlot))
				{
					if(!GC.getSpellInfo((SpellTypes) pLoopUnit->getMissionSpell()).isSpellObsoleted(pLoopUnit, pLoopUnit->getMissionSpell()))
					{
						return true;
					}
				}
