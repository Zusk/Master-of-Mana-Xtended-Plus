...
{
	return m_pPlayer ? new CyPlot(m_pPlayer->findStartingPlot(std::vector<CvPlot*>(0), bRandomize)) : NULL;
}
...
{
	return (m_pPlayer && (m_pPlayer->getLeaderType() != NO_LEADER)) ? m_pPlayer->hasTrait((TraitTypes) iIndex) : false;
}
...
int CyPlayer::getArcaneIncomeTimes100()
{
	return m_pPlayer ? m_pPlayer->getArcaneIncomeTimes100() : -1;
}
