...
CLLNode<IDInfo>* CvAIGroup::prevUnitNode(CLLNode<IDInfo>* pNode) const
{
	if (pNode == NULL)
		return NULL;
...
CLLNode<IDInfo>* CvAIGroup::tailUnitNode() const
{
	return m_units.tail();
}
...
void CvAIGroup::checkForPossibleUpgrade()
{
	for (CLLNode<IDInfo>* pUnitNode = headUnitNode(); pUnitNode != NULL; pUnitNode = nextUnitNode(pUnitNode))
	{
		...
		{
			// When the unit is upgraded the node is broken - SayWhatEh, so we skip causing the following exception
			//break;
			if (nextNode == NULL)
				break;
			pUnitNode = prevUnitNode(nextNode);
		}
	}
}
...
void CvAIGroup::update()
{
	...
	*/
	...
	{
		if (getMissionPlot() != NULL)
			setMissionArea(getMissionPlot()->getArea());
		else if (getHeadUnit() != NULL)
			setMissionArea(getHeadUnit()->plot()->getArea());
		else
		{
			// If terraform or something changed the areas we are without a functioning area here and will crash. Take biggest
			setMissionArea(GC.getMapINLINE().findBiggestArea(false)->getID());
		}
	}
...
void CvAIGroup::update_Reserve_ReinforceCities()
{
    ...
    {
		if (pLoopCity->getArea() == getMissionArea() || (pArea != NULL && pArea->isWater())) // SayWhatEh units walking into water areas that end in the reserve just stand there
		{
...
void CvAIGroup::update_Reserve_ShuffleCityDefense()
{
    ...
    for (CvCityAI* pLoopCity = static_cast<CvCityAI*>(kPlayer.firstCity(&iLoop)); pLoopCity != NULL; pLoopCity = static_cast<CvCityAI*>(kPlayer.nextCity(&iLoop)))
    {
		...
		if (pLoopCity->getArea() == getMissionArea() || pArea->isWater() == true)
		{
			CLLNode<IDInfo>* pNextUnitNode;
...
void CvAIGroup::update_Reserve_ReinforceCounters()
{
	...
	{
		if (pAIGroup->getGroupType() == AIGROUP_COUNTER && (pAIGroup->getMissionArea() == getMissionArea() || pArea->isWater()))
		{
...
void CvAIGroup::update_Reserve_ReinforceDestroyLair()
{
	...
	for(CvAIGroup* pAIGroup = kPlayer.firstAIGroup(&iLoop); pAIGroup != NULL; pAIGroup = kPlayer.nextAIGroup(&iLoop))
	{
		if(pAIGroup->getGroupType()==AIGROUP_DESTROY_LAIR && (pAIGroup->getMissionArea()==getMissionArea() || pArea->isWater()))
		{
			if(pAIGroup->UnitsNeeded()>0)
			{
				if(pAIGroup->canFillWithReserveUnits(this))
...
void CvAIGroup::update_Reserve_ReinforceCityInvasion()
{
	...
	for(CvAIGroup* pAIGroup = kPlayer.firstAIGroup(&iLoop); pAIGroup != NULL; pAIGroup = kPlayer.nextAIGroup(&iLoop))
	{
		if (pAIGroup->getGroupType() == AIGROUP_CITY_INVASION && (pAIGroup->getMissionArea() == getMissionArea() || GC.getMapINLINE().getArea(getMissionArea())->isWater()) && pAIGroup->getMissionTarget() != NULL)
		{
...
void CvAIGroup::update_Reserve_ReinforceManaNode()
{
	...
	for(CvAIGroup* pAIGroup = kPlayer.firstAIGroup(&iLoop); pAIGroup != NULL; pAIGroup = kPlayer.nextAIGroup(&iLoop))
	{
		if (pAIGroup->getGroupType() == AIGROUP_MANA_NODE && (pAIGroup->getMissionArea() == getMissionArea() || GC.getMapINLINE().getArea(getMissionArea())->isWater()))
		{
			if(pAIGroup->UnitsNeeded()>0)
			{
				while(pAIGroup->UnitsNeeded()>0)
...
void CvAIGroup::update_Reserve_ReinforceSiege()
{
	...
	for(CvAIGroup* pAIGroup = kPlayer.firstAIGroup(&iLoop); pAIGroup != NULL; pAIGroup = kPlayer.nextAIGroup(&iLoop))
	{
		if (pAIGroup->getGroupType() == AIGROUP_SIEGE && (pAIGroup->getMissionArea() == getMissionArea() || GC.getMapINLINE().getArea(getMissionArea())->isWater()))
		{
			if(pAIGroup->UnitsNeeded()>0)
			{
				while(pAIGroup->UnitsNeeded()>0)
...
void CvAIGroup::update_Reserve_ReinforceSettlers()
{
	...
	for (CLLNode<IDInfo>* pUnitNode = headUnitNode(); pUnitNode != NULL; pUnitNode = pNextUnitNode)
	{
		...
		if (pLoopUnit->AI_getUnitAIType()==UNITAI_SETTLE)
		{
			...
			pUnitNode = setAIGroupInLoop(pNewGroup, pUnitNode, pLoopUnit);
	...
	for (CLLNode<IDInfo>* pUnitNode = headUnitNode(); pUnitNode != NULL; pUnitNode = pNextUnitNode)
	{
		...
		if (pLoopUnit->AI_getUnitAIType()==UNITAI_EXPLORE)
		{
			...
			for(CvAIGroup* pAIGroup = kPlayer.firstAIGroup(&iLoop); pAIGroup != NULL; pAIGroup = kPlayer.nextAIGroup(&iLoop))
			{
				...
				{
					pUnitNode = setAIGroupInLoop(pAIGroup, pUnitNode, pLoopUnit);
	...
	if(pEquipment!=NULL)
	{
					...
					if(pLoopUnit->generatePath(pEquipment->plot(),0,true,&iPathTurns))
					{
						if(isOOSLogging())
						{
							oosLog("EquipmentPromotion"
								,"Turn: %d, PlayerID: %d,UnitID: %d,generatePath to: %d,Turns: %d\n"
								,GC.getGameINLINE().getElapsedGameTurns()
								,getOwnerINLINE()
								,pLoopUnit->getID()
								,pEquipment->getID()
		...
		if(pBestUnit!=NULL)
		{
			...
			//if(pBestUnit->atPlot(pEquipment->plot()))
			{
				...
				{
					oosLog("EquipmentPromotion"
						,"Turn: %d, PlayerID: %d,UnitID: %d,update_Reserve_PickupEquipment: %d\n"
						,GC.getGameINLINE().getElapsedGameTurns()
						,getOwnerINLINE()
						,pBestUnit->getID()
						,pEquipment->getID()
...
void CvAIGroup::update_Reserve_ExploreDungeon()
{
	...
	{
		pLoopPlot=GC.getMapINLINE().plotByIndex(iI);
		if (pLoopPlot->getArea() == getMissionArea() || GC.getMapINLINE().getArea(getMissionArea())->isWater())
		{
...
void CvAIGroup::update_Worker()
{
	...
	else
	{
		for (CLLNode<IDInfo>* pUnitNode = headUnitNode(); pUnitNode != NULL; pUnitNode = nextUnitNode(pUnitNode))
		{
			...
			pUnitNode = setAIGroupInLoop(NULL, pUnitNode, pLoopUnit);
		}
	}
}

CLLNode<IDInfo>* CvAIGroup::setAIGroupInLoop(CvAIGroup* pNewGroup, CLLNode<IDInfo>* pUnitNode, CvUnit* pLoopUnit)
{
	CLLNode<IDInfo>* nextNode = nextUnitNode(pUnitNode);
	CvAIGroup* pOldGroup = pLoopUnit->getAIGroup();

	pLoopUnit->setAIGroup(pNewGroup);

	if (pOldGroup == pLoopUnit->getAIGroup())
		return pUnitNode; // Same group got set back again
	else if (nextNode != NULL)
		return prevUnitNode(nextNode); // Old node was deleted when we removed the unit from the ai group
	else
		return NULL; // the last last one will do
}
