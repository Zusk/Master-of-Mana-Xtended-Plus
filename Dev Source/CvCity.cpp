...
/*************************************************************************************************/
	...
	m_paiFeatureYieldIncome = new int[NUM_YIELD_TYPES];
...
/*************************************************************************************************/
	...
	SAFE_DELETE_ARRAY(m_paiFeatureYieldIncome);
...
//SpyFanatic: moved above before setPlotCity(this)
...
/*************************************************************************************************/
	...
	// doReligion();  -- disabled, this only spreads Religions to Cities that have none, Sephi
	doGreatPeople();
/*************************************************************************************************/
/**	TIME MEASURE																	Sephi		**/
/*************************************************************************************************/
...
/*************************************************************************************************/
...
#ifdef _DEBUG
	{
		...
		for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
		{
			//FAssert(getBaseYieldRate((YieldTypes)iI) >= 0); //SpyFanatic: can be zero due to negative rate building (tribute to...)
			//FAssert(getYieldRate((YieldTypes)iI) >= 0);
/*if(getBaseYieldRate((YieldTypes)iI)<0){
	oosLog("debugissue","PlayerID:%d,CityID:%d,Yield:%S,base:%d,rate:%d",getOwnerINLINE(),getID(),GC.getYieldInfo((YieldTypes)iI).getDescription(),getBaseYieldRate((YieldTypes)iI),getYieldRate((YieldTypes)iI));
}*/
			iCount = 0;
	...
	case TASK_CITY_DUNGEON_EVENT:
		{
			...
			//SpyFanatic: CTD in case in the meantime AI civ destroyed that Dungeon, so as safe measure, crosscheck that Dungeon is still existing before proceeding, so adding a NULL check
			CvDungeon* pTargetDungeon = GC.getMapINLINE().getDungeon(iData1);
			if(pTargetDungeon != NULL)
			{
				//Do not execute if the dungeon is a barbaric city not anymore belonging to barbs player
				if(pTargetDungeon->plot() == NULL || pTargetDungeon->plot()->getPlotCity() == NULL || pTargetDungeon->plot()->getPlotCity()->getOwnerINLINE() >= BARBARIAN_PLAYER)
				{
					pTargetDungeon->ResolveDeal(getOwnerINLINE(), (DungeonEventTypes)iData2);
				}
			}
		}
		break;
...
bool CvCity::canTrain(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreUpgrades, bool bIgnoreResources) const
{
	if (eUnit == NO_UNIT)
	{
		return false;
...
/**	END	                                        												**/
/*************************************************************************************************/
//BEGIN: GAMEOPTION_NO_WONDER_UNIQUEUNIT 
	if(GC.getGameINLINE().isOption(GAMEOPTION_NO_WONDER_UNIQUEUNIT) &&
	isWorldUnitClass(((UnitClassTypes)(GC.getUnitInfo(eUnit).getUnitClassType())))
	){
		return false;
	}
//END
	...
	if(isHuman() && !bTestVisible) {
		if(GC.getUnitInfo(eUnit).getGlobalYieldCost() > 0 &&
			...
			if(!bContinue) {
				if (GET_PLAYER(getOwnerINLINE()).getGlobalYield(GC.getUnitInfo(eUnit).getGlobalYieldTypeCost()) < getGlobalYieldCost(eUnit)) {
					return false;
				}
			}
		}
    ...
    return canUpgrade(eUnit, bContinue, bTestVisible, bIgnoreCost, bIgnoreUpgrades,bIgnoreResources);
}

bool CvCity::canUpgrade(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreUpgrades, bool bIgnoreResources) const
//FfH: End Modify
...
//FfH: End Add
	...
	if (!plot()->canTrain(eUnit, bContinue, bTestVisible,bIgnoreResources))
	{
		return false;
	}
...
bool CvCity::canConstruct(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreResources) const
{
...
//BEGIN: GAMEOPTION_NO_WONDER_UNIQUEUNIT 
	if(GC.getGameINLINE().isOption(GAMEOPTION_NO_WONDER_UNIQUEUNIT) &&
	isWorldWonderClass((BuildingClassTypes)(GC.getBuildingInfo(eBuilding).getBuildingClassType()))
	){
		return false;
	}
//END
	...
	if (!bTestVisible || !bIgnoreResources)
	{
...
//			if(getProductionTurnsLeft(eBuilding, 0) <= 1) {
				if (GET_PLAYER(getOwnerINLINE()).getGlobalYield(GC.getBuildingInfo(eBuilding).getGlobalYieldTypeCost()) < getGlobalYieldCost(eBuilding)) {
					return false;
				}
			}
		}
	...
	if (!bIgnoreResources)
	{
		for(iI=0;iI<GC.getNumBonusInfos();iI++)
		{
			if(GC.getBuildingInfo(eBuilding).isBonusConsumed(iI))
			{
				...
				if(isOOSLogging())
				{
					oosLog("ConsumeResource"
						,"Turn:%d,Player:%S,Bonus:%S,Available(%d)[+Import(%d)]-Export(%d)-ConsumedByProduction(%d)-CurrentBuildQueue(%d)=%d,old=%d"
						,GC.getGameINLINE().getElapsedGameTurns()
						,GC.getCivilizationInfo(kPlayer.getCivilizationType()).getDescription()
						,GC.getBonusInfo((BonusTypes)iI).getDescription()
						,kPlayer.getNumAvailableBonuses((BonusTypes)iI)
						,kPlayer.getBonusImport((BonusTypes)iI)
						,kPlayer.getBonusExport((BonusTypes)iI)
						,kPlayer.getNumBonusesConsumedByProduction((BonusTypes)iI)
						,iCurrent
						,kPlayer.getNumAvailableBonuses((BonusTypes)iI) - kPlayer.getBonusExport((BonusTypes)iI) - kPlayer.getNumBonusesConsumedByProduction((BonusTypes)iI) + iCurrent
						,kPlayer.getNumAvailableBonuses((BonusTypes)iI)-kPlayer.getBonusImport((BonusTypes)iI)-kPlayer.getNumBonusesConsumedByProduction((BonusTypes)iI)+iCurrent
					);
				}*/
				//SpyFanatic: use imported/exported bonus (available already count for import, and getNumBonusesConsumedByProduction is counting also producion queue so this is why +iCurrent)
				...
				if(kPlayer.getNumAvailableBonuses((BonusTypes)iI) - kPlayer.getBonusExport((BonusTypes)iI) - kPlayer.getNumBonusesConsumedByProduction((BonusTypes)iI) + iCurrent <= 0)
				{
					return false;
				}
			}
...
//FfH: End Add
	...
	if(GC.getGameINLINE().isOption(GAMEOPTION_NO_WONDER_UNIQUEUNIT) && isWorldProject(eProject)
	){
		return false;
	}
...
int CvCity::getBonusYieldRateModifier(YieldTypes eIndex, BonusTypes eBonus) const
{
	...
	if(GC.getBonusInfo(eBonus).getCityYieldModifierType() == (YieldTypes) eIndex)
		iModifier += GC.getBonusInfo(eBonus).getCityBonusModifier();
...
void CvCity::processBonus(BonusTypes eBonus, int iChange)
{
	...
	/*if(eBonus == 46 && iChange == -1 && getBonusGoodHealth() == 2)
	{
		FAssert(eBonus != 46);
	}
	if(isOOSLogging() && iChange != 0)
	{
		oosLog("AIBonuses","Turn:%d,Player:%d,CityID:%d,eBonus:%d,Bonus:%S,processBonus:%d,GoodHealth:%d,BadHealth:%d"
			,GC.getGameINLINE().getElapsedGameTurns()
			,getOwnerINLINE()
			,getID()
			,eBonus
...
//FfH: Added by Kael 01/27/2008
    ...
    if (isCapital())
    {
        //SpyFanatic: removed as water resources in capital are then increasing global spawning rate
		/*
...
/*************************************************************************************************/
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).getTeam() == getTeam())
			{
				...
				{
					GET_PLAYER((PlayerTypes)iI).processBuilding(eBuilding, iChange, area()); //SpyFanatic: turn 218, spawn of mercurians on same city which hold What happen when city is lost due to spawn of mercurians and we just lose the House of Healing? Same team... but health bonus not yet processed by Mercurians
					/*
PlayerID:10,CityName:TXT_KEY_CITY_ELOHIM_2,Building:House of Healing,num:0,oldnum:1
PlayerID:10,changeBuildingGoodHealth:1,change:-1,after:0
PlayerID:12,changeBuildingGoodHealth:0,change:-1,after:-1
					*/
				}
			}
		}
...
int CvCity::happyLevel() const
{
	...
	iHappiness += std::max(0, getAITurnHappinessBonus());
...
void CvCity::changeBonusGoodHealth(int iChange)
{
	if (iChange != 0)
	{
		...
		//FAssert(getBonusGoodHealth() >= 0); //SpyFanatic:same assert as below
...
void CvCity::changeBonusBadHealth(int iChange)
{
	if (iChange != 0)
	{
		...
		//FAssert(getBonusBadHealth() <= 0); //SpyFanatic:same assert as below
...
int CvCity::getAITurnHappinessBonus() const
{
	if (!isHuman()) {
		int iInterval = GC.getHandicapInfo(GC.getGameINLINE().getHandicapType()).getAIFreeHappinessInterval();
		if (iInterval > 0) {
...
//		FAssert(getYieldRate(eIndex) >= 0);		//For the Sidar during the turn when the AI resets specialists this gets set to -1 because their palace adds +1
...
void CvCity::calculateCityBonusYieldRateModifier()
{
	...
	for(int i = 0; i < NUM_YIELD_TYPES; ++i) {
		...
		//Certain buildings give bonuses.  For instance the Hippus palace gives a bonus horse so bonuses from buildings need to be added here.
		for (int k = 0; k < GC.getNumBuildingInfos(); k++) {
			if (getNumBuilding((BuildingTypes) k) > 0) {
				if (GC.getBuildingInfo((BuildingTypes) k).getNumFreeBonuses() > 0) {
					if (GC.getBuildingInfo((BuildingTypes) k).getFreeBonus() != NO_BONUS && GC.getBonusInfo((BonusTypes) GC.getBuildingInfo((BuildingTypes) k).getFreeBonus()).getCityYieldModifierType() == (YieldTypes) i)
						iModifier += GC.getBonusInfo((BonusTypes) GC.getBuildingInfo((BuildingTypes) k).getFreeBonus()).getCityBonusModifier();
					if (GC.getBuildingInfo((BuildingTypes)k).getFreeBonus2() != NO_BONUS && GC.getBonusInfo((BonusTypes) GC.getBuildingInfo((BuildingTypes) k).getFreeBonus2()).getCityYieldModifierType() == (YieldTypes) i)
						iModifier += GC.getBonusInfo((BonusTypes) GC.getBuildingInfo((BuildingTypes) k).getFreeBonus2()).getCityBonusModifier();
					if (GC.getBuildingInfo((BuildingTypes)k).getFreeBonus3() != NO_BONUS && GC.getBonusInfo((BonusTypes) GC.getBuildingInfo((BuildingTypes) k).getFreeBonus3()).getCityYieldModifierType() == (YieldTypes) i)
						iModifier += GC.getBonusInfo((BonusTypes) GC.getBuildingInfo((BuildingTypes) k).getFreeBonus3()).getCityBonusModifier();
				}
			}
		}
...
void CvCity::updateExtraSpecialistYield(YieldTypes eYield)
{
	...
	{	
		FAssert(iNewYield >= -100);
		FAssert(iNewYield <= 10000);
		//There is an issue that hasn't been figured out where citizen specialist production yields will overflow and get set to the max.  
		//As a temporary workaround, if the code wants us to set that kind of a value, just set it to be 2 - asserts are still in place to track it.
		if (iNewYield >= -100 && iNewYield <= 10000) {	
...
//FfH: End Modify
	...
	{
		iBaseCommerceRate += calculateSpellResearchFromFeaturesTimes100();
	}
...
int CvCity::getTotalCommerceRateModifier(CommerceTypes eIndex) const
{
	return std::max(0, (getCommerceRateModifier(eIndex) 
		...
		+ (GET_PLAYER(getOwnerINLINE()).getStateReligion() == NO_RELIGION ? 0 : GET_PLAYER(getOwnerINLINE()).getStateReligionCommerceTechRateModifier(eIndex)));
}
...
void CvCity::calculateCityBonusCommerceRateModifier()
{
	...
	for(int i = 0; i < NUM_COMMERCE_TYPES; ++i) {
		...
		//Certain buildings give bonuses.  For instance the Khazad palace gives a bonus gold resource so bonuses from buildings need to be added here.
		for (int k = 0; k < GC.getNumBuildingInfos(); k++) {
			if (getNumBuilding((BuildingTypes)k) > 0) {
				if (GC.getBuildingInfo((BuildingTypes)k).getNumFreeBonuses() > 0) {
					if (GC.getBuildingInfo((BuildingTypes)k).getFreeBonus() != NO_BONUS && GC.getBonusInfo((BonusTypes)GC.getBuildingInfo((BuildingTypes)k).getFreeBonus()).getCityCommerceModifierType() == (CommerceTypes)i){
						iModifier += GC.getBonusInfo((BonusTypes)GC.getBuildingInfo((BuildingTypes)k).getFreeBonus()).getCityBonusModifier();
					}
					if (GC.getBuildingInfo((BuildingTypes)k).getFreeBonus2() != NO_BONUS && GC.getBonusInfo((BonusTypes)GC.getBuildingInfo((BuildingTypes)k).getFreeBonus2()).getCityCommerceModifierType() == (CommerceTypes)i){
						iModifier += GC.getBonusInfo((BonusTypes)GC.getBuildingInfo((BuildingTypes)k).getFreeBonus2()).getCityBonusModifier();
					}
					if (GC.getBuildingInfo((BuildingTypes)k).getFreeBonus3() != NO_BONUS && GC.getBonusInfo((BonusTypes)GC.getBuildingInfo((BuildingTypes)k).getFreeBonus3()).getCityCommerceModifierType() == (CommerceTypes)i){
						iModifier += GC.getBonusInfo((BonusTypes)GC.getBuildingInfo((BuildingTypes)k).getFreeBonus3()).getCityBonusModifier();
					}
				}
			}
...
{
	/*if(isOOSLogging() && getCultureTimes100(eIndex) + iChange < 0)
	{
		oosLog("AINegativeCulture"
				,"Turn:%d,Player:%d,CityID:%d,Culture:%d,Change:%d,Founded:%d\n"
				,GC.getGameINLINE().getElapsedGameTurns()
				,getOwner()
				,getID()
				,getCultureTimes100(eIndex)
				,iChange
				,getGameTurnFounded()
			);
	}*/
...
//FfH: Added by Kael 08/21/2007
		...
		{
            //processBonus(eIndex, iChange); //SpyFanatic: this is only for the below Mana Bonus topic, not for health or else...
			if (GC.getBonusInfo(eIndex).getMutateChance() != 0)
			{
				changeMutateChance(GC.getBonusInfo(eIndex).getMutateChance() * iChange);
			}
			if (GC.getBonusInfo(eIndex).getResearchModifier() != 0)
			{
				changeCommerceRateModifier(COMMERCE_RESEARCH, GC.getBonusInfo(eIndex).getResearchModifier() * iChange);
			}
		}
//FfH: End Add
...
{
	/**
	GC.getTimeMeasure().Start("CvCityAI::AI_addBestCitizen::AI_specialistValue::isSpecialistValid::first");
	int first = getSpecialistCount(eIndex) + iExtra;
	GC.getTimeMeasure().Stop("CvCityAI::AI_addBestCitizen::AI_specialistValue::isSpecialistValid::first");
	GC.getTimeMeasure().Start("CvCityAI::AI_addBestCitizen::AI_specialistValue::isSpecialistValid::second");
	int second = getMaxSpecialistCount(eIndex);
	GC.getTimeMeasure().Stop("CvCityAI::AI_addBestCitizen::AI_specialistValue::isSpecialistValid::second");
	GC.getTimeMeasure().Start("CvCityAI::AI_addBestCitizen::AI_specialistValue::isSpecialistValid::third");
	int third =  GET_PLAYER(getOwnerINLINE()).isSpecialistValid(eIndex);
	GC.getTimeMeasure().Stop("CvCityAI::AI_addBestCitizen::AI_specialistValue::isSpecialistValid::third");
	GC.getTimeMeasure().Start("CvCityAI::AI_addBestCitizen::AI_specialistValue::isSpecialistValid::fourth");
	int fourth = GC.getDefineINT("DEFAULT_SPECIALIST");
	GC.getTimeMeasure().Stop("CvCityAI::AI_addBestCitizen::AI_specialistValue::isSpecialistValid::fourth");

	return ((first <= second) || third || (eIndex == fourth));
	*/
	...
	return (((getSpecialistCount(eIndex) + iExtra) <= getMaxSpecialistCount(eIndex)) || GET_PLAYER(getOwnerINLINE()).isSpecialistValid(eIndex) || (eIndex == GC.getDEFAULT_SPECIALIST()));
}
...
void CvCity::changeUnitCombatFreeExperience(UnitCombatTypes eIndex, int iChange)
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < GC.getNumUnitCombatInfos(), "eIndex expected to be < GC.getNumUnitCombatInfos()");
	FAssertMsg(iChange < 1000, "change is expected to be something reasonable");
	FAssertMsg(iChange > -1000, "change is expected to be something reasonable");
	...
	if (isWorkingPlot(iIndex) != bNewValue)
	{
		...
		if (pPlot != NULL)
		{
			...
			if (isWorkingPlot(iIndex))
			{
				...
				for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
				{
					changeBaseYieldRate(((YieldTypes)iI), pPlot->getYield((YieldTypes)iI), true);
				}
			...
			if(getGlobalYieldCost((UnitTypes)iData1) > 0 &&
				GC.getUnitInfo((UnitTypes)iData1).getGlobalYieldTypeCost() != NO_YIELD) {
					if(isHuman()) {
						...
						//SpyFanatic: put discount on xml alt defines... Trying to set to 0 to see impact for acquiring equipments
						int iChange = std::max(pPlayer.getGlobalYield(eYield) - GC.getDefineINT("AI_DISCOUNT_GLOBAL_YIELD"), 0);
		...
		if (canConstruct((BuildingTypes)iData1) || bForce)
		{
			...
			for(int iI=0;iI<GC.getNumBonusInfos();iI++)
			{
				...
				if(kBuilding.isBonusConsumed(iI))
				{
					...
					//oosLog("consumeBonus","%d %S 1",iI,GC.getBonusInfo((BonusTypes)iI).getDescription()); //,GET_PLAYER(getOwnerINLINE()).getNumBonusesConsumedByProduction((BonusTypes)iI)
				}
			}
...
/*************************************************************************************************/
	...
	pStream->Read(NUM_YIELD_TYPES, m_paiFeatureYieldIncome);
...
/*************************************************************************************************/
	...
	pStream->Write(NUM_YIELD_TYPES, m_paiFeatureYieldIncome);
...
bool CvCity::isValidBuildingLocation(BuildingTypes eBuilding) const
{
	...
	if(kBuilding.isReqGlobalYieldIncome())
	{
		for(int iI=YIELD_COMMERCE+1;iI<GC.getNUM_YIELD_TYPES();iI++)
		{
			...
			{
				if(getYieldRate((YieldTypes)iI)<=0)
				{
					return false;
				}
			}
...
int CvCity::getNumCityPlots() const
{
    ...
    return NUM_CITY_PLOTS_STANDARD;
}
...
void CvCity::setPlotRadius(int iNewValue)
{
    if (iNewValue != getPlotRadius())
    {
		...
		int iOldPlotRadius = m_iPlotRadius;
        m_iPlotRadius = iNewValue;
        //SpyFanatic: added to manage plot radius increase plot from 2 to 3 and avoid issue setPlotCity for Kuriotates
		if (iOldPlotRadius != m_iPlotRadius)
        {
			for (iI = iOldPlotRadius; iI < NUM_CITY_PLOTS; ++iI)
			{
				pLoopPlot = plotCity(getX_INLINE(), getY_INLINE(), iI);
				...
				if (pLoopPlot != NULL)
				{
					...
					if(isOOSLogging())
					{
						oosLog("AICityRadius","Turn:%d,Player:%d,CityID:%d,X:%d,Y:%d,AdditionalChange:%d,m_iCityRadiusCount:%d"
								,GC.getGameINLINE().getElapsedGameTurns()
								,getOwner()
								,getID()
								,pLoopPlot->getX()
								,pLoopPlot->getY()
								,(iOldPlotRadius < m_iPlotRadius) ? 1 : -1
								,pLoopPlot->getCityRadiusCount()
							);
					}
					*/

					pLoopPlot->changeCityRadiusCount((iOldPlotRadius < m_iPlotRadius) ? 1 : -1);
					pLoopPlot->changePlayerCityRadiusCount(getOwnerINLINE(), (iOldPlotRadius < m_iPlotRadius) ? 1 : -1);
				}
			}
		}
		//
...
int CvCity::getAdditionalCommerceTimes100ByBuilding(CommerceTypes eIndex, BuildingTypes eBuilding) const
{
	...
	if (getNumActiveBuilding(eBuilding) > 0)	//If the city already has the building, then the formula needs to be adjusted to properly calculate how much the building is actually giving.
		return ((iExtraModifier * iExtraRate) + (iExtraModifier * (iRateTimes100 - iExtraRate)) + ((iModifier - iExtraModifier) * iExtraRate)) / 100;
	else 
		return ((iModifier + iExtraModifier) * (iExtraRate + iRateTimes100) / 100) - (iModifier * iRateTimes100 / 100);
}
...
void CvCity::changeYieldFromBonus(int iYield,int iChange)
{
	if(iChange!=0)
	{
		...
		changeBaseYieldRate((YieldTypes)iYield,-iOldValue/100, true);
		changeBaseYieldRate((YieldTypes)iYield,iNewValue/100, true);
	}
}
        ...
        while (pUnitNode != NULL)
        {
            ...
            if (pLoopUnit != NULL)
            {
				for(int i = 0; i < NUM_COMMERCE_TYPES; ++i) {
					...
					//SpyFanatic: make sure that Balseraph Puppet does not cause negative culture or research
					m_paiCommerceFromUnits[i] = std::max(0,m_paiCommerceFromUnits[i]);
				}
			}
		}
	}
...
int CvCity::getAdditionalYieldByBuilding(YieldTypes eIndex, BuildingTypes eBuilding) const
{
	...
	int iExtraRate = getAdditionalBaseYieldRateByBuilding(eIndex, eBuilding);
	int iExtraModifier = getAdditionalYieldRateModifierByBuilding(eIndex, eBuilding);
	if (iExtraRate == 0 && iExtraModifier == 0)
	{
		return 0;
	}
	...
	int iRate = getBaseYieldRate(eIndex) + iUnhappyProd;
	int iModifier = getBaseYieldRateModifier(eIndex);

	if (getNumActiveBuilding(eBuilding) > 0)	//If the city already has the building, then the formula needs to be adjusted to properly calculate how much the building is actually giving.
	{
/*
		oosLog("debugissue","PlayerID:%d,CityID:%d,Yield:%S,extrarate:%d,extramodifier:%d,rate:%d,modifier:%d,unhappy:%d,poweryield:%d,area:%d",getOwnerINLINE(),getID(),GC.getYieldInfo(eIndex).getDescription(),iExtraRate,iExtraModifier,iRate,iModifier,unhappyLevel(0), isPower() ? getPowerYieldRateModifier(eIndex) : -666,(area() != NULL) ? area()->getYieldRateModifier(getOwnerINLINE(), eIndex) : -666);
*/
		if(iExtraRate < 0)
		{
			//SpyFanatic: for negative rate from a building, modifier should be added.. not subtracted! PS: as of now there is no building who provide negative extra rate and extra modifiers... So the formula has been simplified
			//Tribute to overloard -1 production, tribute to lugus -1 gold, kilmore -1 culture, junil -1 spellreasearch
			//Building that decrease percentage: energy drain, defiled soil -20 % production. ash rain -20% food production commerce
			return ((200 - iModifier) * iExtraRate) / 100;
		}

		return ((iExtraModifier * iExtraRate) + (iExtraModifier * (iRate - iExtraRate)) + ((iModifier - iExtraModifier) * iExtraRate)) / 100;
	}
	else
	{
		return (((iModifier + iExtraModifier) * (iExtraRate + iRate)) - (iModifier * iRate)) / 100;
	}
}
...
int CvCity::getAdditionalYieldRateModifierByBuilding(YieldTypes eIndex, BuildingTypes eBuilding) const
{
	...
	if (!bObsolete)
	{
		...
		**/
		for (int iI = 0; iI < GC.getNumBonusInfos(); ++iI)	//Go through the bonuses and if the city has a bonus (like iron) for a building (like a forge), then apply the modifier from that building.
		{
			if (hasBonus((BonusTypes)iI))
			{
		...
		if (GC.getBuildingInfo(eBuilding).getNumFreeBonuses() > 0) {	//Buildings that produce yield bonuses directly should have it noted that the city production will increase 
			if (GC.getBuildingInfo(eBuilding).getFreeBonus() != NO_BONUS && GC.getBonusInfo((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus()).getCityYieldModifierType() == eIndex && !hasBonus((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus()))
				iExtraModifier += GC.getBonusInfo((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus()).getCityBonusModifier();
			if (GC.getBuildingInfo(eBuilding).getFreeBonus2() != NO_BONUS && GC.getBonusInfo((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus2()).getCityYieldModifierType() == eIndex && !hasBonus((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus2()))
				iExtraModifier += GC.getBonusInfo((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus2()).getCityBonusModifier();
			if (GC.getBuildingInfo(eBuilding).getFreeBonus3() != NO_BONUS && GC.getBonusInfo((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus3()).getCityYieldModifierType() == eIndex && !hasBonus((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus3()))
				iExtraModifier += GC.getBonusInfo((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus3()).getCityBonusModifier();
		}
	}
...
int CvCity::getAdditionalBaseCommerceRateByBuilding(CommerceTypes eIndex, BuildingTypes eBuilding) const
{
	...
	int iExtraRate = getAdditionalBaseCommerceRateByBuildingTimes100Impl(eIndex, eBuilding);
	if (bNoEspionage && eIndex == COMMERCE_CULTURE)
	{
		iExtraRate += getAdditionalBaseCommerceRateByBuildingTimes100Impl(COMMERCE_ESPIONAGE, eBuilding);
	}
	return iExtraRate;
}
 ...
 * Doesn't check if the building can be constructed in this city.
 */
int CvCity::getAdditionalBaseCommerceRateByBuildingTimes100Impl(CommerceTypes eIndex, BuildingTypes eBuilding) const
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < NUM_COMMERCE_TYPES, "eIndex expected to be < NUM_COMMERCE_TYPES");
	FAssertMsg(eBuilding >= 0, "eBuilding expected to be >= 0");
	...
	if (!bObsolete)
	{
		...
		|| getCommercePercentTypeForced()==eIndex)
		{
			...
			int iCommercechange = (getAdditionalBaseYieldRateByBuilding(YIELD_COMMERCE, eBuilding) * (iModifier + getAdditionalYieldRateModifierByBuilding(YIELD_COMMERCE, eBuilding)));
		...
		if(kBuilding.getCommercePercentTypeForced()!=NO_COMMERCE)
		{
			...
			int iCityCommerceRate = (getAdditionalBaseYieldRateByBuilding(YIELD_COMMERCE, eBuilding) * (iModifier + getAdditionalYieldRateModifierByBuilding(YIELD_COMMERCE, eBuilding)));
...
int CvCity::getAdditionalCommerceRateModifierByBuildingImpl(CommerceTypes eIndex, BuildingTypes eBuilding) const
{
	...
	{
		//Certain buildings give bonuses.  For instance the Khazad palace gives a bonus gold resource so bonuses from buildings need to be added here.
		if (GC.getBuildingInfo(eBuilding).getNumFreeBonuses() > 0) {
			if (GC.getBuildingInfo(eBuilding).getFreeBonus() != NO_BONUS && GC.getBonusInfo((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus()).getCityCommerceModifierType() == (CommerceTypes)eIndex && !hasBonus((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus()))
				iExtraModifier += GC.getBonusInfo((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus()).getCityBonusModifier();
			if (GC.getBuildingInfo(eBuilding).getFreeBonus2() != NO_BONUS && GC.getBonusInfo((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus2()).getCityCommerceModifierType() == (CommerceTypes)eIndex && !hasBonus((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus2()))
				iExtraModifier += GC.getBonusInfo((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus2()).getCityBonusModifier();
			if (GC.getBuildingInfo(eBuilding).getFreeBonus3() != NO_BONUS && GC.getBonusInfo((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus3()).getCityCommerceModifierType() == (CommerceTypes)eIndex && !hasBonus((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus3()))
				iExtraModifier += GC.getBonusInfo((BonusTypes)GC.getBuildingInfo(eBuilding).getFreeBonus3()).getCityBonusModifier();
		}
...
int CvCity::getFeatureYieldIncome(int iI) const
{
	if (iI < 0 || iI > GC.getNUM_YIELD_TYPES())
	{
		return 0;
	}
    return std::max(0, m_paiFeatureYieldIncome[iI]);
}
...
int CvCity::getFeatureYieldIncomeChange(int iYield) const {
	int iIncome = 0;
	CvPlot* pLoopPlot;
	...
	int iOldIncome = getFeatureYieldIncome(iYield);
	if (iYield == YIELD_LUMBER)
		m_paiFeatureYieldIncome[iYield] = ((iIncome * (100 + getYieldRateModifier((YieldTypes)iYield))) / 200);
	else
		m_paiFeatureYieldIncome[iYield] = ((iIncome * (100 + getYieldRateModifier((YieldTypes)iYield))) / 100);

	return m_paiFeatureYieldIncome[iYield] - iOldIncome;
}

void CvCity::updateFeatureYieldIncome(int iYield)
{
	changeBaseYieldRate((YieldTypes)iYield, getFeatureYieldIncomeChange(iYield), true);
}
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
...
/*
	If the number of bonuses changes, anything that is dependent on the number of bonuses should be calculated here. 
*/
...
//	changeBaseYieldRate((YieldTypes)iYieldType, -getYieldFromBonus(iYieldType)/100, true);
	changeYieldFromBonus(iYieldType,-getYieldFromBonus(iYieldType));	//This also updates the base yield rate.
	//reset to zero
...
//	}
	...
	return GC.getUnitInfo(eUnit).getGlobalYieldCost() * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getTrainPercent() / 100;
}
...
int CvCity::getGlobalYieldCost(BuildingTypes eBuilding) const
{
	...
	return (GC.getBuildingInfo(eBuilding).getGlobalYieldCost() * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getConstructPercent()) / 100;
}
...
int CvCity::getStepDistance(CvPlot* pTargetPlot)
{
	gDLL->getFAStarIFace()->ForceReset(&GC.getStepUnitFinder());
	gDLL->getFAStarIFace()->GeneratePath(&GC.getStepUnitFinder(), getX_INLINE(), getY_INLINE(), pTargetPlot->getX_INLINE(), pTargetPlot->getY_INLINE(), false, 0, false);
	FAStarNode* pNode = gDLL->getFAStarIFace()->GetLastNode(&GC.getStepUnitFinder());
	if(pNode != NULL)
	{
