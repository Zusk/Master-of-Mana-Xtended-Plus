...
**/
			if (!(pLoopPlot->isWater()) && 
				((pLoopPlot->getPlane() != NO_PLANE && !GC.getPlaneInfo(pLoopPlot->getPlane()).isNoRiver()) || GC.getMapINLINE().useClimateBasedStartPos()))
			{
...
void CvMapGenerator::addFeatures()
{
	...
	for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		for (int iJ = 0; iJ < GC.getNumFeatureInfos(); iJ++)
		{
			if (GC.getMapINLINE().useClimateBasedStartPos() && (FeatureTypes)iJ == iIce)
				continue; // Ice Ice Never!
...
void CvMapGenerator::generateRandomMap()
{
	...
	if (GC.getMapINLINE().useClimateBasedStartPos()) // Climate based terrain types and starting positions, no planes
	{
		GC.getMapINLINE().changeTerrainBasedOnPlayers();
		return;
	}
	else
...
void CvMapGenerator::addUniqueManaLocation(ImprovementTypes eImprovement)
{
	...
	for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if(!pPlot->isWater() && !pPlot->isPeak() && !pPlot->isStartingPlot())
		{
			...
			{
				if(GC.getMapINLINE().useClimateBasedStartPos() || !GC.getMap().isPlanes() || (pPlot->getPlane() == GC.getImprovementInfo(eImprovement).getNativePlane()))
				{
					if(GC.getImprovementInfo(eImprovement).getTerrainMakesValid(pPlot->getTerrainType()))
					{
						...
						if(bValid)
						{
							if(GC.getGameINLINE().getMapRandNum(100, "addUniqueManaLocations") < 25) {
