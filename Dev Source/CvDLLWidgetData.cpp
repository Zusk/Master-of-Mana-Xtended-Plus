...
{
	if (widgetDataStruct.m_iData1 == NO_TECH)
	{
		//	No Technology
		if (GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCurrentResearch() != NO_TECH)
		{
			CyArgsList argsList;
			argsList.add(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCurrentResearch());
		...
		if (GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType() != NO_MISSION)
		{
			...
			if (GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType() == MISSION_HEAL)
			{
				...
				while (pSelectedUnitNode != NULL)
				{
					...
					//SpyFanatics; when selectiongroup has some units that are hurt but cannot heal (e.g. plagued) MAX_INT turns is displayed
					int iUnitTurns = pSelectedUnit->healTurns(pMissionPlot);
					if(iUnitTurns > 0 && iUnitTurns < MAX_INT)
					{
						iTurns = std::max(iTurns, iUnitTurns);
					}
...
//FfH: End Add
				...
				if (NO_IMPROVEMENT != eImprovement)
				{
					...
					if (iHealth < 0 && GET_PLAYER(pHeadSelectedUnit->getOwner()).isIgnoreUnHealthFromImprovements())
						iHealth = 0;
				...
				if (eImprovement != NO_IMPROVEMENT)
				{
					...
					else
					{
						...
						for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
						{
							...
							{
								if (pMissionPlot->getPossibleLandBonusDiscoverChance((BonusTypes)iI, pHeadSelectedUnit->getOwnerINLINE(), GC.getImprovementInfo(eImprovement)))
								{
					...
					if (GC.getImprovementInfo(eImprovement).getImprovementUpgrade() != NO_IMPROVEMENT && 
						((GC.getImprovementInfo((ImprovementTypes)GC.getImprovementInfo(eImprovement).getImprovementUpgrade()).getPrereqCivilization() == NO_PLAYER) ||
						(GC.getImprovementInfo((ImprovementTypes) GC.getImprovementInfo(eImprovement).getImprovementUpgrade()).getPrereqCivilization() != NO_PLAYER &&
							pMissionPlot->getOwnerINLINE() != NO_PLAYER && 
							GC.getImprovementInfo((ImprovementTypes)GC.getImprovementInfo(eImprovement).getImprovementUpgrade()).getPrereqCivilization() == GET_PLAYER(pMissionPlot->getOwnerINLINE()).getCivilizationType())
						))
					{
...
**/
	...
	{
		if(pLoopCity->getYieldRate(eYield)>0)
		{
			szTempBuffer.Format(L"%c %d",GC.getYieldInfo(eYield).getChar(), pLoopCity->getYieldRate(eYield));
			szBuffer.append(szTempBuffer);
			szBuffer.append(gDLL->getText("TXT_KEY_MISC_CITY_PRODUCE_GLOBALYIELD",pLoopCity->getNameKey()));
			szBuffer.append(NEWLINE);

			iTotalIncome+=pLoopCity->getYieldRate(eYield);
		}
	}
/**
