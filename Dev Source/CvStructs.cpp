...
void TargetUnitSpellInfo::calculateSpellDamage(CvUnit* attacker, CvUnit* target, CvSpellInfo& kSpell, CvPlot* targetPlot) {
	iDmg = kSpell.getDamage() * 10 +
		(GET_PLAYER(attacker->getOwner()).getBonusSpellDamage(attacker->getMissionSpell()) > 0 ? (GET_PLAYER(attacker->getOwner()).getBonusSpellDamage(attacker->getMissionSpell()) * 10) : 0);
	totalDamage = iDmg + (kSpell.getDamageType() != DAMAGE_PHYSICAL ? attacker->getSpellDamageModify() : 0);
	resistance = (target->getUnitInfo().getTier() - 1) * 10 + target->getDamageTypeResist((DamageTypes)kSpell.getDamageType()) + target->getLevel() + target->getCityResistance();

	groupMemberAdjustment = attacker->isFlamingArrows() ? totalDamage - resistance : target->getSpellDamageGroupAdjustment((totalDamage - resistance) * 10) / 100;

	int maxDamageTotal = groupMemberAdjustment * 3;
	maxDamageTotalCap = 0;
	lowDamageCap = 0;

	if (groupMemberAdjustment + target->getDamage() > kSpell.getDamageLimit()) {
		lowDamageCap = groupMemberAdjustment + target->getDamage() - kSpell.getDamageLimit();
	}

	if (maxDamageTotal + target->getDamage() > kSpell.getDamageLimit())
		maxDamageTotalCap = maxDamageTotal + target->getDamage() - kSpell.getDamageLimit();

	int critChanceTotal = attacker->getSpellCritChance(kSpell);
	int expectedDamageTotal = std::min(kSpell.getDamageLimit(), std::max(0, (int)(groupMemberAdjustment - lowDamageCap + ((maxDamageTotal - maxDamageTotalCap - groupMemberAdjustment + lowDamageCap) * (critChanceTotal / 100.0)))));

	resistChance = target->getResistChance(attacker, (SpellTypes)attacker->getMissionSpell());

	nonAdjustedResistChance = GC.getDefineINT("SPELL_RESIST_CHANCE_BASE");
	nonAdjustedResistChance += target->getLevel() * 5;
	nonAdjustedResistChance += target->getResist();
	nonAdjustedResistChance += attacker->getResistModify();
	nonAdjustedResistChance += GC.getSpellInfo((SpellTypes)attacker->getMissionSpell()).getResistModify();
	nonAdjustedResistChance += GET_PLAYER(target->getOwnerINLINE()).getResistModify();
	nonAdjustedResistChance += GET_PLAYER(attacker->getOwnerINLINE()).getResistEnemyModify();

	CvPlot* pAdjacentPlot;
	totalUnitsAffectedBySecondaryDamage = 0;
	totalDamageExpectedInflictedBySecondaryDamage = 0;
	if (kSpell.getTriggerSecondaryPlotChance() > 0)
	{
		for (int iI = 0; iI < NUM_DIRECTION_TYPES; ++iI)
		{
			pAdjacentPlot = plotDirection(targetPlot->getX_INLINE(), targetPlot->getY_INLINE(), ((DirectionTypes)iI));

			if (pAdjacentPlot != NULL && pAdjacentPlot->isVisible(attacker->getTeam(), false) && attacker->castDamage((SpellTypes)attacker->getMissionSpell(), targetPlot, true) > 0)
			{
				CLLNode<IDInfo>* pUnitNode;
				pUnitNode = pAdjacentPlot->headUnitNode();
				CvUnit* targetAdjacent;

				while (pUnitNode != NULL)
				{
					targetAdjacent = ::getUnit(pUnitNode->m_data);
					pUnitNode = pAdjacentPlot->nextUnitNode(pUnitNode);
					if (pAdjacentPlot->getNumVisibleEnemyDefenders(attacker) > 0 && targetAdjacent->isEnemy(attacker->getTeam(), pAdjacentPlot)) {
						int iDmg = kSpell.getDamage() * 10 +
							(GET_PLAYER(attacker->getOwner()).getBonusSpellDamage(attacker->getMissionSpell()) > 0 ? (GET_PLAYER(attacker->getOwner()).getBonusSpellDamage(attacker->getMissionSpell()) * 10) : 0);
						int totalDamage = iDmg + (kSpell.getDamageType() != DAMAGE_PHYSICAL ? attacker->getSpellDamageModify() : 0);
						int iResistance = (targetAdjacent->getUnitInfo().getTier() - 1) * 10 + targetAdjacent->getDamageTypeResist((DamageTypes)kSpell.getDamageType()) + targetAdjacent->getLevel() + targetAdjacent->getCityResistance();
						int groupMemberAdjustment = attacker->isFlamingArrows() ? (totalDamage - iResistance) : targetAdjacent->getSpellDamageGroupAdjustment((totalDamage - iResistance) * 10) / 100;

						int maxDamageTotal = groupMemberAdjustment * 3;
						int maxDamageTotalCap = 0;

						if (maxDamageTotal + target->getDamage() > kSpell.getDamageLimit())
							maxDamageTotalCap = maxDamageTotal + target->getDamage() - kSpell.getDamageLimit();

						int critChanceTotal = attacker->getSpellCritChance(kSpell);
						int expectedDamageTotal = std::min(kSpell.getDamageLimit(), std::max(0, (int)(groupMemberAdjustment - lowDamageCap + ((maxDamageTotal - maxDamageTotalCap - groupMemberAdjustment) * (critChanceTotal / 100.0)))));

						totalDamageExpectedInflictedBySecondaryDamage += (kSpell.getTriggerSecondaryPlotChance() * (expectedDamageTotal));
					}
				}

				totalUnitsAffectedBySecondaryDamage += pAdjacentPlot->getNumVisibleEnemyDefenders(attacker);
			}
		}
	}
	totalDamageExpectedInflictedBySecondaryDamage /= 100;

	lowDamage = std::max(0, groupMemberAdjustment - lowDamageCap);
	expectedDamage = expectedDamageTotal;
	highDamage = std::max(0, maxDamageTotal - maxDamageTotalCap);
}
