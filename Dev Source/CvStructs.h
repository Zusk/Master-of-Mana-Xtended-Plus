...
struct TargetUnitSpellInfo {
	public:
		CvUnit* m_pTargetedUnit;
		...
		int totalUnitsAffectedBySecondaryDamage;
		int totalDamageExpectedInflictedBySecondaryDamage;
		int groupMemberAdjustment;
		int nonAdjustedResistChance;

		int lowDamageCap;
		int maxDamageTotalCap;
		...
		void calculateSpellDamage(CvUnit* attacker, CvUnit* target, CvSpellInfo& kSpell, CvPlot* targetPlot);
};

struct AttackerSpellInfo {
	public:
		CvUnit* m_pSelectedUnit;
		std::list<TargetUnitSpellInfo> targetUnits;
		int expectedDamageTotal;
		int highDamageTotal;
		int lowDamageTotal;
		int manaCost;
		CvSpellInfo* kSpell;
