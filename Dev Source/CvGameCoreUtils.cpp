...
/**						                                            							**/
/*************************************************************************************************/
	case UNITAI_ACHERON: szString = L"ACHERON"; break;
	case UNITAI_ANIMALDEFENDER: szString = L"ANIMALDEFENDER"; break;
/*************************************************************************************************/
/**	END	                                        												**/
/*************************************************************************************************/
...
std::list<AttackerSpellInfo> calculateAttackerSpellInfo(CLLNode<IDInfo>* pSelectedUnitNode, CvUnit* pSelectedUnit, CvPlot * pTargetplot, CvPlayerAI& kPlayer) {
	std::list<AttackerSpellInfo> attackingUnitsInformation;
	SpellTypes spellType;
	int iMana = kPlayer.getMana();
	...
	while (pSelectedUnitNode != NULL || singleAttacker) {
		...
		int damageTotal = 0;
		if (!(pSelectedUnit->canCast(spellType, false))) {			//If a unit can't cast a spell (say the spell is meteor swarm, but an adept is included in the selection group), don't factor them in.
			if (singleAttacker)
				break;
			else 
				pSelectedUnitNode = gDLL->getInterfaceIFace()->nextSelectionListNode(pSelectedUnitNode);
		...
		AttackerSpellInfo attackerSpellInfo;
		attackerSpellInfo.m_pSelectedUnit = pSelectedUnit;
		attackerSpellInfo.kSpell = &GC.getSpellInfo(spellType);

		CLLNode<IDInfo>* pUnitNode = pTargetplot->headUnitNode();
		std::vector<CvUnit*> bestDefenders;							//Just because a unit is the "top" unit displayed doesn't mean it is ideal for casting the spell - there is math for that.
		while (pUnitNode != NULL) {
			bestDefenders.push_back(::getUnit(pUnitNode->m_data));
			pUnitNode = pTargetplot->nextUnitNode(pUnitNode);
		}

		for (size_t i = 0; i < bestDefenders.size(); i++) {
			int swaps = 0;
			for (size_t j = 0; j < bestDefenders.size() - i - 1; j++) {
				if (pSelectedUnit->getSpellDefenderValue(bestDefenders[j + 1], pTargetplot, attackerSpellInfo.kSpell->getDamageType()) > pSelectedUnit->getSpellDefenderValue(bestDefenders[j], pTargetplot, attackerSpellInfo.kSpell->getDamageType())) {
					iter_swap(bestDefenders.begin() + j, bestDefenders.begin() + j + 1);
					swaps = 1;
				}
			}
			if (!swaps)
		...
		for (std::vector<CvUnit*>::iterator it = bestDefenders.begin(); it != bestDefenders.end() && (pSelectedUnit->getSpellTargetNumber(*attackerSpellInfo.kSpell) + (pSelectedUnit->getSpellTargetIncrease() % 100 != 0 ? 1u : 0u) > attackerSpellInfo.targetUnits.size()); it++) {
			CvUnit* nextTargetUnit = *it;

			TargetUnitSpellInfo targetUnitSpellInfo;
			targetUnitSpellInfo.m_pTargetedUnit = nextTargetUnit;
			targetUnitSpellInfo.calculateSpellDamage(pSelectedUnit, nextTargetUnit, *attackerSpellInfo.kSpell, pTargetplot);
			if (pSelectedUnit->getSpellTargetIncrease() % 100 != 0 && (pSelectedUnit->getSpellTargetNumber(*attackerSpellInfo.kSpell) + (pSelectedUnit->getSpellTargetIncrease() % 100 != 0 ? 1 : 0) == attackerSpellInfo.targetUnits.size() + 1)) {
				targetUnitSpellInfo.expectedDamage = (targetUnitSpellInfo.expectedDamage * (pSelectedUnit->getSpellTargetIncrease() % 100)) / 100;
				targetUnitSpellInfo.possibleTarget = true;
			}

			attackerSpellInfo.targetUnits.push_back(targetUnitSpellInfo);
		}

		attackingUnitsInformation.push_back(attackerSpellInfo);
	...
	while (it != attackingUnitsInformation.end()){
		bool erase = false;
		if (it->kSpell->getManaCost() > 0 && iMana < kPlayer.getSpellManaCost(spellType, it->m_pSelectedUnit)) {
			erase = true;
			iMana -= it->kSpell->getManaCost();
		}
		if (it->kSpell->getFaithCost() > 0 && iFaith < it->kSpell->getFaithCost()) {
			erase = true;
			iFaith -= it->kSpell->getFaithCost();
		}
		if(erase){
			it = attackingUnitsInformation.erase(it);
		}else{
			++it;
		}
	}
	/*
	for (std::list<AttackerSpellInfo>::iterator it = attackingUnitsInformation.begin(); it != attackingUnitsInformation.end(); it++) {

		if (it->kSpell->getManaCost() > 0) {
			if (iMana < kPlayer.getSpellManaCost(spellType, it->m_pSelectedUnit))
				attackingUnitsInformation.erase(it);
			iMana -= it->kSpell->getManaCost();
		}

		if (it->kSpell->getFaithCost() > 0) {
			if (iFaith < it->kSpell->getFaithCost())
				attackingUnitsInformation.erase(it);
			iFaith -= it->kSpell->getFaithCost();
		}
	}
	*/
	for (std::list<AttackerSpellInfo>::iterator attackerIt = attackingUnitsInformation.begin(); attackerIt != attackingUnitsInformation.end(); attackerIt++) {
		std::list<TargetUnitSpellInfo> targets = attackerIt->targetUnits;
		for (std::list<TargetUnitSpellInfo>::iterator targetIt = targets.begin(); targetIt != targets.end(); targetIt++) {
			attackerIt->expectedDamageTotal += targetIt->expectedDamage;
			attackerIt->highDamageTotal += targetIt->highDamage;
			attackerIt->lowDamageTotal += targetIt->lowDamage;
		}

		attackerIt->expectedDamageTotal += targets.begin()->totalDamageExpectedInflictedBySecondaryDamage;
	}
...
void oosLog(CONST TCHAR* szLogName,CONST TCHAR* szFmt, ...)
{
	...
	{
		sprintf(szFilename,"%s-%d%s",szLogName,GC.getGameINLINE().getActivePlayer(),".log");
	}

	TCHAR szOut[1024];
	va_list arg;
	va_start (arg, szFmt);
  	vsprintf(szOut,szFmt, arg);
  	va_end (arg);

	gDLL->logMsg(szFilename,szOut, false, false);
}
//SpyFanatic: OOSLogging functions END
