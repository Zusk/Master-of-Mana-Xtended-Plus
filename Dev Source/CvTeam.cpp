...
//	int iBestValue;
	...
	if (isHasTech(eIndex) != bNewValue)
	{
		...
		if (isHasTech(eIndex))
		{
			...
			/*************************************************************************************************/
			// If you have the tech required to for religion, you now get the holy city! - SayWhatEh 20150314
			if (bFirst && GET_PLAYER(ePlayer).getNumCities() > 0)
			{
				for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
				{
					if (GC.getReligionInfo((ReligionTypes)iI).getTechPrereq() == eIndex && GC.getGameINLINE().getHolyCity((ReligionTypes)iI) == 0)
					{
						CvCity* bestCity = NULL;
						int bestFaith = -1000;

						// We just got the tech needed and there is no holy city. Find one of ours to make it
						for (int iCityNum = 0; iCityNum < GET_PLAYER(ePlayer).getNumCities(); iCityNum++)
						{
							CvCity* city = GET_PLAYER(ePlayer).getCity(iCityNum);
							if (city == NULL)
								continue;
							int faith = city->getCommerceRate(COMMERCE_FAITH);
							if (faith > bestFaith)
							{
								bestCity = city;
								bestFaith = faith;
						...
						GC.getGameINLINE().setHolyCity((ReligionTypes)iI, bestCity, bAnnounce);
					}
				}
			}
			/*************************************************************************************************/
...
/**	ADDON (Houses of Erebus) Sephi			                                 					**/
			...
			{
				if (kPlayer.getStateReligion() != NO_RELIGION && GC.getTechInfo(eTech).getReligionCommerceTechRateModifier(kPlayer.getStateReligion(), iJ) != 0)
				{
