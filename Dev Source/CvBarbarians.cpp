...
void CvPlayer::barb_spawn_group_new_lair()
{
	...
	if (GuardianUnitType == NO_UNIT)
		return;

	int cityIndex = GC.getGameINLINE().getSorenRandNum(getNumCities() - 1, "pick city?");
	int index = 0;
	CvCity* pBarbCity = firstCity(&index);

	if (pBarbCity == NULL)
		return;

	for (int i = 0; i < cityIndex; i++)
		pBarbCity = nextCity(&index);

	if (pBarbCity == NULL)
		return;

	if (pBarbCity->plot()->getNumUnits() > 4)
		return;
...
void CvPlayer::barb_spawn_new_lair()
{
	bool keepChecking = true;
	int tries = 0;
	while (keepChecking && tries < 100)
	{
		tries++;
		int index = GC.getGameINLINE().getSorenRandNum(GC.getMapINLINE().numPlotsINLINE(), "getting index for new dungeon");
		CvPlot* pPlot = GC.getMapINLINE().plotByIndexINLINE(index);
...
void CvPlayer::spendBarbarianPoints()
{
	...
	//if(getBarbarianPower()<500) {
		while(getBarbarianPoints() > 1500) {
			...
			int iAction = GC.getGameINLINE().getSorenRandNum(3, "spendBarbarianPoints");
