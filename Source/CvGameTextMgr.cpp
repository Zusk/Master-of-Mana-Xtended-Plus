...
//>>>>	More Detailed Civilopedia: Added by Denev 2009/09/05
...
//BUGFfH: Added by Denev 2009/09/06
		...
		/********************************************************/
			...
			if (bSummoningAbility)
			{
				...
				bFirst = true;
				for (std::list<CvWString>::iterator it = aszPromotionSummonPerk.begin(); it != aszPromotionSummonPerk.end(); it++)
				{
					szTempBuffer.Format(L"%s%s ", NEWLINE, gDLL->getText("TXT_KEY_PROMOTION_SUMMON_PERK_HEADER").GetCString());
					setListHelp(szString, szTempBuffer, *it, L", ", bFirst);
					bFirst = false;
				}
			}
...
//					szString.append(gDLL->getText("TXT_KEY_UNIT_STRENGTH", pUnit->terrainAttackModifier((TerrainTypes)iI), GC.getTerrainInfo((TerrainTypes) iI).getTextKeyWide()));
					szString.append(gDLL->getText("TXT_KEY_UNIT_STRENGTH", pUnit->terrainAttackModifier((TerrainTypes)iI), getLinkedText((TerrainTypes)iI).c_str()));
//<<<<	BUGFfH: End Modify
...
//					szString.append(gDLL->getText("TXT_KEY_UNIT_ATTACK", pUnit->terrainAttackModifier((TerrainTypes)iI), GC.getTerrainInfo((TerrainTypes) iI).getTextKeyWide()));
					szString.append(gDLL->getText("TXT_KEY_UNIT_ATTACK", pUnit->terrainAttackModifier((TerrainTypes)iI), getLinkedText((TerrainTypes)iI).c_str()));
//<<<<	BUGFfH: End Modify
...
//					szString.append(gDLL->getText("TXT_KEY_UNIT_DEFENSE", pUnit->terrainDefenseModifier((TerrainTypes)iI), GC.getTerrainInfo((TerrainTypes) iI).getTextKeyWide()));
					szString.append(gDLL->getText("TXT_KEY_UNIT_DEFENSE", pUnit->terrainDefenseModifier((TerrainTypes)iI), getLinkedText((TerrainTypes)iI).c_str()));
//<<<<	BUGFfH: End Modify
...
//					szString.append(gDLL->getText("TXT_KEY_UNIT_STRENGTH", pUnit->featureAttackModifier((FeatureTypes)iI), GC.getFeatureInfo((FeatureTypes) iI).getTextKeyWide()));
					szString.append(gDLL->getText("TXT_KEY_UNIT_STRENGTH", pUnit->featureAttackModifier((FeatureTypes)iI), getLinkedText((FeatureTypes)iI).c_str()));
//<<<<	BUGFfH: End Modify
...
//					szString.append(gDLL->getText("TXT_KEY_UNIT_ATTACK", pUnit->featureAttackModifier((FeatureTypes)iI), GC.getFeatureInfo((FeatureTypes) iI).getTextKeyWide()));
					szString.append(gDLL->getText("TXT_KEY_UNIT_ATTACK", pUnit->featureAttackModifier((FeatureTypes)iI), getLinkedText((FeatureTypes)iI).c_str()));
//<<<<	BUGFfH: End Modify
...
//					szString.append(gDLL->getText("TXT_KEY_UNIT_DEFENSE", pUnit->featureDefenseModifier((FeatureTypes)iI), GC.getFeatureInfo((FeatureTypes) iI).getTextKeyWide()));
					szString.append(gDLL->getText("TXT_KEY_UNIT_DEFENSE", pUnit->featureDefenseModifier((FeatureTypes)iI), getLinkedText((FeatureTypes)iI).c_str()));
//<<<<	BUGFfH: End Modify
...
*/
				if (pUnit->getDamageTypeResist((DamageTypes)iI) != 0)
				{
					...
					{
						szString.append(gDLL->getText("TXT_KEY_PROMOTION_DAMAGE_TYPE_IMMUNE", GC.getDamageTypeInfo((DamageTypes)iI).getTextKeyWide()));
					}
					else
					{
						szString.append(gDLL->getText("TXT_KEY_PROMOTION_DAMAGE_TYPE_RESIST", pUnit->getDamageTypeResist((DamageTypes)iI), GC.getDamageTypeInfo((DamageTypes)iI).getTextKeyWide()));
					}
				}
			}
...
*/
			iModifier = pPlot->defenseModifier(pDefender->getTeam(), (pAttacker != NULL) ? pAttacker->ignoreBuildingDefense() : true);
			if (!pDefender->noDefensiveBonus() || iModifier < 0)
			{
		...
		for (iI = 0; iI < GC.getNumBuildingClassInfos(); ++iI)
		{
			...
			if (eLoopBuilding != NO_BUILDING && !isWorldWonderClass((BuildingClassTypes)iI))
			{
				if(kTrait.isIgnoreBuildingGYCost(iI)) {
					...
					setListHelp(szHelpString, szText.GetCString(), szBuilding, L", ", (kTrait.isIgnoreBuildingGYCost(iI) != iLast));
...
//								GC.getUnitInfo(eUniqueUnit).getDescription());
							szBuffer.Format(L"%s - (%s)",
								...
								bLinks ? getLinkedText(eUniqueUnit).c_str() : GC.getUnitInfo(eDefaultUnit).getDescription());
//<<<<	BUGFfH: End Modify
						}
						else
						{
...
*/
							szBuffer.Format(L"%s  %c%s - (%s)", NEWLINE,
											...
											getLinkedText(eUniqueUnit, false).c_str());
//<<<<	BUGFfH: End Modify
						}
						else
						{
...
void CvGameTextMgr::parsePromotionHelp(CvWStringBuffer &szBuffer, PromotionTypes ePromotion, const wchar* pcNewline)
{
    ...
    if (kPromotion.getManaCost() > 0)
    {
		szBuffer.append(pcNewline);
		CvWString szTempBuffer;
        szTempBuffer.append(gDLL->getText("TXT_KEY_PROMOTION_MANA_COST", GC.getPromotionInfo(ePromotion).getManaCost()));
		if(GC.getGameINLINE().getActivePlayer()!=NO_PLAYER && GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getMana()<GC.getPromotionInfo(ePromotion).getManaCost() && gDLL->getInterfaceIFace()->getHeadSelectedUnit()!=NULL)
			szTempBuffer.Format( SETCOLR L"%s" ENDCOLR , TEXT_COLOR("COLOR_NEGATIVE_TEXT"), szTempBuffer.c_str());
		szBuffer.append(szTempBuffer);
	}
    ...
    if (kPromotion.getFaithCost() > 0)
    {
		szBuffer.append(pcNewline);
		CvWString szTempBuffer;
        szTempBuffer.append(gDLL->getText("TXT_KEY_PROMOTION_FAITH_COST", GC.getPromotionInfo(ePromotion).getFaithCost()));
		if(GC.getGameINLINE().getActivePlayer()!=NO_PLAYER && GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getFaith()<GC.getPromotionInfo(ePromotion).getFaithCost() && gDLL->getInterfaceIFace()->getHeadSelectedUnit()!=NULL)
			szTempBuffer.Format( SETCOLR L"%s" ENDCOLR , TEXT_COLOR("COLOR_NEGATIVE_TEXT"), szTempBuffer.c_str());
		szBuffer.append(szTempBuffer);
	}
...
//>>>>	More Detailed Civilopedia: Added by Denev 2009/11/10
	if (GC.getUnitInfo(eUnit).isDefensiveStrikes())
	{
		szBuffer.append(NEWLINE);
		szBuffer.append(gDLL->getText("TXT_KEY_UNIT_DEFENSIVE_STRIKES"));
	}
...
/*************************************************************************************************/
    for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
	{
        if (GC.getUnitInfo(eUnit).getBonusAptitude((BonusTypes)iI) != 0)
        {
            szBuffer.append(NEWLINE);
            szBuffer.append(gDLL->getText("TXT_KEY_BONUS_APTITUDE",
                            GC.getUnitInfo(eUnit).getBonusAptitude((BonusTypes)iI),
                            getLinkedText((BonusTypes)iI).c_str()));
        }
	}
