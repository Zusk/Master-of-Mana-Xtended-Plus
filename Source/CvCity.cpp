...
//FfH: End Add
	...
	//new formula
	int iCap = getPopulation() * 100;

	iCap *= std::max(0, (GET_PLAYER(getOwnerINLINE()).getDistanceMaintenanceModifier() + 100));
	iCap /= 100;

	iTempMaintenance = std::min(getPopulation() * 100, iTempMaintenance);
...
int CvCity::calculateCityYieldIncome(int iYield) const
{
	...
	for (int iI = 0; iI < getNumCityPlots(); iI++)
	{
		...
		if (pLoopPlot != NULL)
		{
			if (pLoopPlot->getWorkingCity() == this)
			{
				if(pLoopPlot->getImprovementType() != NO_IMPROVEMENT && pLoopPlot != plot())
				{
