...
int CvPlayer::findStartingArea() const
{
	...
	for(pLoopArea = GC.getMapINLINE().firstArea(&iLoop); pLoopArea != NULL; pLoopArea = GC.getMapINLINE().nextArea(&iLoop))
	{
		if (!(pLoopArea->isWater())
			&& pLoopArea->getPlane() == GC.getCivilizationInfo(getCivilizationType()).getNativePlane())
		{
...
**/
        ...
        if (GC.getUnitInfo(eUnit).getStateReligion()==RELIGION_COUNCIL_OF_ESUS)
        {
            isEsus=true;
            if (!isFollowEsus())
            {
...
//FfH Civics: Added by Kael 08/11/2007
	...
	if (GC.getCivicInfo(eCivic).isPreservation())
	{
		if(doCivicPreservation(true)==0)
		{
			return false;
		}
	}
...
/** Esus isn't allowed State Religion Anymore and Empy isn't allowed State Religion for Esus Followers **/
    ...
    if (isFollowEsus())
    {
        if (eReligion==RELIGION_THE_EMPYREAN)
        {
            return false;
        }
    }
...
//added Sephi - simplified Civic Upkeep
	int iUnmodified=std::max(0,(getNumCities()-1- getNumSettlements())*(2*GC.getCivicInfo(eCivic).getUpkeep()+1));	
...
void CvPlayer::doSummonRitual(ProjectTypes eProject)
{
	...
	if(GC.getProjectInfo(eProject).getSummonUnitType()==NO_UNIT)
	{
		return;
	}
...
/**	New Esus                                                                 					**/
bool CvPlayer::isFollowEsus() const
{
    return m_bFollowEsus;
}

void CvPlayer::setFollowEsus(bool bNewValue)
{
	if(m_bFollowEsus!=bNewValue)
	{
		m_bFollowEsus=bNewValue;
		if(bNewValue)
		{
			changeManaCreationModifier(-30);
		}
		else
		{
			changeManaCreationModifier(30);
...
int CvPlayer::CalculateManaByNodes() const
{
	...
	int iMod = 0;
	int iMagic2 = GC.getInfoTypeForString("TECH_SORCERY");
	int iMagic3 = GC.getInfoTypeForString("TECH_ARCANE_LORE");
	int iMagic4 = GC.getInfoTypeForString("TECH_STRENGTH_OF_WILL");

	if (iMagic2 != NO_TECH && GET_TEAM(getTeam()).isHasTech((TechTypes)iMagic2))
	{
		iMod++;
	}

	if (iMagic3 != NO_TECH && GET_TEAM(getTeam()).isHasTech((TechTypes)iMagic3))
	{
		iMod++;
	}

	if (iMagic4 != NO_TECH && GET_TEAM(getTeam()).isHasTech((TechTypes)iMagic4))
	{
		iMod++;
	}
...
bool CvPlayer::canEquipUnit(const CvUnit* pUnit, PromotionTypes Equipment, bool bTestVisible, bool bTreasure) const
{
	...
	if(kPromotion.getGoldCost()>getGold())
	{
		return false;
	}

	if(kPromotion.getManaCost()>getMana())
	{
		return false;
	}

	if(kPromotion.getFaithCost()>getFaith())
	{
		return false;
	}
