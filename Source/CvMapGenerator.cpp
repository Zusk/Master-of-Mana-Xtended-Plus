	...
	for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if(!pPlot->isWater() && !pPlot->isPeak() && !pPlot->isStartingPlot())
		{
			if(pPlot->getBonusType()==NO_BONUS && pPlot->getImprovementType()==NO_IMPROVEMENT)
			{
				...
				if(bValid)
				{
					if(GC.getGameINLINE().getMapRandNum(100, "addUniqueManaLocations") < 20) {
						if(pPlot->getPlane() == GC.getImprovementInfo(iLostTemple).getNativePlane()) {
							if(GC.getImprovementInfo(iLostTemple).getTerrainMakesValid(pPlot->getTerrainType())) {
								pPlot->setImprovementType(iLostTemple);
								pPlot->setFeatureType(NO_FEATURE);
							}
						}
						else if(pPlot->getPlane() == GC.getImprovementInfo(iWitchHut).getNativePlane()) {
							if(GC.getImprovementInfo(iWitchHut).getTerrainMakesValid(pPlot->getTerrainType())) {
								pPlot->setImprovementType(iWitchHut);
								pPlot->setFeatureType(NO_FEATURE);
							}
						}
						else if(pPlot->getPlane() == GC.getImprovementInfo(iManaShrine).getNativePlane()){
							if(GC.getImprovementInfo(iManaShrine).getTerrainMakesValid(pPlot->getTerrainType())) {
								pPlot->setImprovementType(iManaShrine);
								pPlot->setFeatureType(NO_FEATURE);
							}
						}
					}
