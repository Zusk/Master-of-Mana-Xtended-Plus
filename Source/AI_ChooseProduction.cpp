...
//District AI
	...
	/**
	if(GC.getBuildingInfo(eBuilding).isDistrict() && !isHasDistrict())
	{
		...
		{
			if(eBuildingClass != GC.getBUILDINGCLASS_SAGE_DISTRICT())
			{
				return -100000;
			}
		}
		else if(AI_getCitySpecialization() == CITYSPECIALIZATION_GOLD)
		{
			if(eBuildingClass != GC.getBUILDINGCLASS_MERCHANT_DISTRICT())
			{
				return -100000;
			}
		}
	...
	if(GC.getBuildingInfo(eBuilding).isDistrict()) {
		BuildingTypes eSageDistrict = (BuildingTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(GC.getBUILDINGCLASS_SAGE_DISTRICT());
		if(getNumRealBuilding(eSageDistrict) > 0) {
			if(AI_getCitySpecialization() == CITYSPECIALIZATION_RESEARCH)
			{
				if(eBuilding != GC.getInfoTypeForString("BUILDING_SALON")) {
					return -100000;
				}
			}
			if(AI_getCitySpecialization() == CITYSPECIALIZATION_ARCANE)
			{
				if(eBuilding != GC.getInfoTypeForString("BUILDING_SCHOLA_ARCANA")) {
					return -100000;
				}
			}
		}
...
**/
	...
	{
		if(GET_PLAYER(getOwnerINLINE()).getFavoriteReligion() == RELIGION_THE_ASHEN_VEIL)
		{
			 return 10000;
		}
	}
...
int CvCityAI::AI_ValueUnitBaseCombat(UnitTypes eUnit, bool bOnlyDefense, bool bWater)
{
	...
	for(int iI=0;iI<GC.getNumBonusInfos();iI++)
	{
		...
		if(kUnit.getBonusAptitude(iI)!=0)
		{
			if(GET_PLAYER(getOwnerINLINE()).getNumAvailableBonuses((BonusTypes)iI)>0)
			{
...
int CvCityAI::AI_ValueCityCounter(UnitTypes eUnit)
{
	...
	if(kUnit.getUnitCombatType()==GC.getInfoTypeForString("UNITCOMBAT_RANGED") && eUnit==GC.getUnitClassInfo((UnitClassTypes)kUnit.getUnitClassType()).getDefaultUnitIndex())
	{
		return -MAX_INT;
	}
...
int CvCityAI::AI_ValueCityInvasionUnit(CvAIGroup* pAIGroup, UnitTypes eUnit)
{
	...
	if(kUnit.getUnitCombatType()==GC.getInfoTypeForString("UNITCOMBAT_MELEE"))
	{
		iValue*=120;
		iValue/=100;
	}
...
bool CvCityAI::AI_chooseProductionBarbarian()
{
	...
	{
		if (eBestUnit==GC.getInfoTypeForString("UNIT_ACHERON") && getUnitProduction(eBestUnit)<1)
		{
			for(int iJ=0;iJ<GC.getMAX_PLAYERS();iJ++)
			{
				if(GET_PLAYER((PlayerTypes)iJ).isAlive() && GET_PLAYER((PlayerTypes)iJ).isHuman())
				{
					szBuffer = gDLL->getText("TXT_KEY_MESSAGE_START_PRODUCTION_ACHERON", getNameKey());
					gDLL->getInterfaceIFace()->addMessage((PlayerTypes)iJ, true, GC.getEVENT_MESSAGE_TIME(), szBuffer, 0, MESSAGE_TYPE_MINOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"), getX_INLINE(), getY_INLINE(), true, true);
				}
			}
		}
...
bool CvCityAI::AI_chooseCitySiegeUnit()
{
	...
	if(GET_PLAYER(getOwnerINLINE()).getCivilizationType()==GC.getInfoTypeForString("CIVILIZATION_LUCHUIRP"))
	{
		int iYeahWhatever=0;
	}
