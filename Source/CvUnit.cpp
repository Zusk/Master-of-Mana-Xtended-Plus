...
/**						                                            							**/
/*************************************************************************************************/
	m_paiBonusAptitude = NULL;
	m_paiBonusAptitudeAmount = NULL;
...
/*************************************************************************************************/
    for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
    {
        changeBonusAptitude((BonusTypes)iI, m_pUnitInfo->getBonusAptitude((BonusTypes)iI));
    }
...
/**						                                            							**/
/*************************************************************************************************/
	SAFE_DELETE_ARRAY(m_paiBonusAptitude);
	SAFE_DELETE_ARRAY(m_paiBonusAptitudeAmount);
...
//<<<<Advanced Rules: End Modify
	...
	m_iBaseCombatDefenseCarryOver = 0;
...
/*************************************************************************************************/
	...
	if (!bConstructorCall)
	{
        m_paiBonusAptitude = new int[GC.getNumBonusInfos()];
        m_paiBonusAptitudeAmount = new int[GC.getNumBonusInfos()];
        for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
        {
            m_paiBonusAptitude[iI] = 0;
            m_paiBonusAptitudeAmount[iI] = 0;
        }
...
//FfH: End Add
		...
		case COMMAND_MOBILIZE:
			...
			break;
		case COMMAND_USE_PORTAL:
			setXY(plot()->getPortalExit()->getX_INLINE(), plot()->getPortalExit()->getY_INLINE());
			finishMoves();
			break;
		default:
			FAssert(false);
...
/** Check for Esus Cult instead of State Religion                                               **/
/**						                                            							**/
/*************************************************************************************************/
/** orig
...
**/
        if (GET_PLAYER(getOwnerINLINE()).isFollowEsus())
        {
            return true;
        }
/*************************************************************************************************/
	...
	if (pPlot->getImprovementType() != NO_IMPROVEMENT)
	{
			...
			GET_PLAYER(pPlot->getOwner()).isAlive())
		{
			...
			if(eYield != NO_YIELD) {
				...
				if (iPlunder > 0)
				{
					int iPlunderDuration = 10 + GC.getGameINLINE().getSorenRandNum(20, "Plunder Time"); 
...
/*************************************************************************************************/
/**	ADDON (New Esus) Sephi                                                    					**/
/** Block Esus from Upgrading to Tier4 Units                                                    **/
/**						                                            							**/
/*************************************************************************************************/
    if (GET_PLAYER(getOwnerINLINE()).isFollowEsus())
    {
        if(GC.getUnitInfo(getUnitType()).getTier()==4)
        {
            int iDisciple=GC.getInfoTypeForString("UNITCOMBAT_DISCIPLE");
            if(iDisciple!=NO_UNITCOMBAT && GC.getUnitInfo(getUnitType()).getUnitCombatType())
            {
                return false;
            }
        }
...
{
	int rawStrength = m_iBaseCombat;

	for (int iI = 0; iI < GC.getNumPromotionInfos(); iI++)
	{
		PromotionTypes prom = (PromotionTypes)iI;

		if (isHasPromotion(prom)){

			CvPromotionInfo &kPromotion = GC.getPromotionInfo(prom);
			rawStrength -= kPromotion.getExtraCombatStr();
...
{
	int rawStrength = m_iBaseCombatDefense;
	...
	for (int iI = 0; iI < GC.getNumPromotionInfos(); iI++)
	{
		...
		if (isHasPromotion(prom)){
			...
			CvPromotionInfo &kPromotion = GC.getPromotionInfo(prom);
			rawStrength -= kPromotion.getExtraCombatDefense();
...
/** new Promotion tags **/
	if(GC.getPromotionInfo(ePromotion).isClassPromotion() && getLevel()<7)
	{
		return false;
	}
...
**/
        ...
        int iEsus = GC.getInfoTypeForString("RELIGION_COUNCIL_OF_ESUS");
        bool isEsus=false;

        if (iEsus != NO_RELIGION && GC.getPromotionInfo(ePromotion).getStateReligionPrereq()==iEsus)
        {
            isEsus=true;
            if (!GET_PLAYER(getOwnerINLINE()).isFollowEsus())
            {
...
/*************************************************************************************************/
	...
	if(!(kSpell.isTargetedHostile() && pTargetPlot==NULL))
	{
		...
		if(!canAttack() && pTargetPlot!=plot()) {
			int iXPCap = kSpell.getManaCost() * 2;
			int iChange = std::min(3, iXPCap - getExperience());
			iChange = std::max(0, iChange);
...
{
	//Non combat units with default strength zero shouldn't gain strength (that excludes jotnar workers)
	if(iChange == 0 || (getUnitCombatType() == NO_UNITCOMBAT && m_pUnitInfo->getCombat() == 0))
		return;

	int iNewStrength = m_iBaseCombat + iChange;

	//Strength shouldn't be below zero so we use a help field to store the value below zero (carryover)
	if(iChange < 0){
		if(m_iBaseCombatCarryOver < 0){
			m_iBaseCombatCarryOver += iChange;
		}else{
			if(iNewStrength < 0){
				m_iBaseCombat = 0;
				m_iBaseCombatCarryOver = iNewStrength;
			}else{
		...
		if(m_iBaseCombatCarryOver < 0){
			iChange += m_iBaseCombatCarryOver;
			if(iChange <= 0){
				m_iBaseCombatCarryOver = iChange;
			}else{
				m_iBaseCombatCarryOver = 0;
...
{
	//Non combat units with default strength zero shouldn't gain strength (that excludes jotnar workers)
	if(iChange == 0 || (getUnitCombatType() == NO_UNITCOMBAT && m_pUnitInfo->getCombatDefense() == 0))
		return;

	int iNewDefStrength = m_iBaseCombatDefense + iChange;

	//Strength shouldn't be below zero so we use a help field to store the value below zero (carryover)
	if(iChange < 0){
		if(m_iBaseCombatDefenseCarryOver < 0){
			m_iBaseCombatDefenseCarryOver += iChange;
		}else{
			if(iNewDefStrength < 0){
				m_iBaseCombatDefense = 0;
				m_iBaseCombatDefenseCarryOver = iNewDefStrength;
			}else{
		...
		if(m_iBaseCombatDefenseCarryOver < 0){
			iChange += m_iBaseCombatDefenseCarryOver;
			if(iChange <= 0){
				m_iBaseCombatDefenseCarryOver = iChange;
			}else{
				m_iBaseCombatDefenseCarryOver = 0;
...
/*************************************************************************************************/
	...
	pStream->Read(GC.getNumBonusInfos(), m_paiBonusAptitude);
	pStream->Read(GC.getNumBonusInfos(), m_paiBonusAptitudeAmount);
...
//<<<<Refined Defensive Strikes: End Add
	...
	//Carryovers
	pStream->Read(&m_iBaseCombatCarryOver);
	pStream->Read(&m_iBaseCombatDefenseCarryOver);
}
...
/*************************************************************************************************/
	...
	pStream->Write(GC.getNumBonusInfos(), m_paiBonusAptitude);
	pStream->Write(GC.getNumBonusInfos(), m_paiBonusAptitudeAmount);
...
//<<<<Refined Defensive Strikes: End Add
	...
	//Carryovers
	pStream->Write(m_iBaseCombatCarryOver);
	pStream->Write(m_iBaseCombatDefenseCarryOver);
}
...
void CvUnit::changeBonusAptitude(BonusTypes eIndex, int iChange)
{
    if (iChange != 0)
    {
        m_paiBonusAptitude[eIndex] += iChange;
    }
    updateBonusAptitude(eIndex);
}

int CvUnit::getBonusAptitude(BonusTypes eIndex) const
{
	return m_paiBonusAptitude[eIndex];
}

void CvUnit::updateBonusAptitude(BonusTypes eIndex)
{
    int iNew = std::min(1,GET_PLAYER(getOwnerINLINE()).getNumAvailableBonuses(eIndex)) * getBonusAptitude(eIndex);
    int iOld = m_paiBonusAptitudeAmount[eIndex];
    if (GC.getBonusInfo(eIndex).getDamageType() == NO_DAMAGE)
    {
        m_iBaseCombat += iNew - iOld;
        m_iBaseCombatDefense += iNew - iOld;
    ...
    m_paiBonusAptitudeAmount[eIndex] = iNew;
}
...
void CvUnit::updatePower()
{
	...
	// Calculate bonuses from affinities and aptitudes
	for (int iI = 0; iI < GC.getNumBonusInfos(); iI++)
	{
		...
		int iAptitude = getBonusAptitude((BonusTypes)iI);
...
int CvUnit::getDefensiveStrikeBonus(const CvUnit* pTarget) const
{
	...
	if(getDefensiveStrikeDamage()==0 || !hasDefensiveStrikesLeft())
	{
		return 0;
	}
